...
public class ProgressDialogImplementation implements IProgress {
	...
	public ProgressDialogImplementation(Context ctx, ProgressDialog dlg,
			boolean cancelable) {
	...
	public static ProgressDialogImplementation createProgressDialog(
			Context ctx, String title, String message, int style) {
		ProgressDialog dlg = new ProgressDialog(ctx);
		dlg.setTitle(title);
		dlg.setMessage(message);
		dlg.setIndeterminate(style == ProgressDialog.STYLE_HORIZONTAL); // re-set
																		// in
																		// mViewUpdateHandler.handleMessage
																		// above
		dlg.setCancelable(true);
		// we'd prefer a plain progress bar without numbers,
		// but that is only available starting from API level 11
		try {
			ProgressDialog.class.getMethod("setProgressNumberFormat",
					new Class[] { String.class }).invoke(dlg, (String) null);
	...
	public void progress(int deltaWork) {
		if (!isIndeterminate() && dialog != null) {
			...
			if ((deltaProgress > (work / 100))
					|| ((progress + deltaProgress) >= work)) {
	...
	public void finishTask() {
		...
		progress = 0;
		if (taskName != null) {
			message = context.getResources().getString(R.string.finished_task)
					+ " : " + taskName; //$NON-NLS-1$
			mViewUpdateHandler.sendEmptyMessage(HANDLER_START_TASK);
		}
	}
