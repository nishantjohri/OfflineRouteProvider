...
public class BaseLocationIndexRepository<T extends MapObject> {
	private static final Log log = OsmLogUtil
			.getLog(BaseLocationIndexRepository.class);
	...
	public boolean initialize(final IProgress progress, File file, int version,
			String tableLocation, boolean searchX31) {
		...
		Cursor cursor = db
				.rawQuery(
						"SELECT name FROM sqlite_master WHERE type='table' AND name='" + metaTable + "'", null); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (dbExist) {
			cursor = db
					.rawQuery(
							"SELECT MAX_LAT, MAX_LON, MIN_LAT, MIN_LON  FROM " + metaTable, null); //$NON-NLS-1$
			...
			try {
				db
						.execSQL("CREATE TABLE " + metaTable + " (MAX_LAT DOUBLE, MAX_LON DOUBLE, MIN_LAT DOUBLE, MIN_LON DOUBLE)"); //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (searchX31) {
				Cursor query = db
						.query(
								tableLocation,
								new String[] {
										"MIN(y)", "MAX(x)", "MAX(y)", "MIN(x)" }, null, null, null, null, null); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
				if (query.moveToFirst()) {
					dataTopLatitude = MapUtils.get31LatitudeY(query.getInt(0)) + 1;
					dataRightLongitude = MapUtils.get31LongitudeX(query
							.getInt(1)) + 1.5;
					dataBottomLatitude = MapUtils.get31LatitudeY(query
							.getInt(2)) - 1;
					dataLeftLongitude = MapUtils.get31LongitudeX(query
							.getInt(3)) - 1.5;
				}
				query.close();
			} else {
				Cursor query = db
						.query(
								tableLocation,
								new String[] {
										"MAX(latitude)", "MAX(longitude)", "MIN(latitude)", "MIN(longitude)" }, null, null, null, null, null); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
				if (query.moveToFirst()) {
	...
	protected void updateMaxMinBoundaries(String tableLocation) {
		...
		db
				.execSQL(
						"INSERT INTO " + metatable + " VALUES (?, ?, ? ,?)", new Double[] { dataTopLatitude, dataRightLongitude, dataBottomLatitude, dataLeftLongitude }); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public synchronized boolean checkCachedObjects(double topLatitude,
			...
			int zoom, List<T> toFill, boolean fillFound) {
		if (db == null) {
			return true;
		}
	...
	public boolean checkContains(double latitude, double longitude) {
		if (latitude <= dataTopLatitude && latitude >= dataBottomLatitude
				&& longitude >= dataLeftLongitude
				&& longitude <= dataRightLongitude) {
			return true;
		}
		return false;
	}
	...
	public boolean checkContains(double topLatitude, double leftLongitude,
		...
		if (rightLongitude < dataLeftLongitude
				|| leftLongitude > dataRightLongitude) {
			return false;
		}
		if (topLatitude < dataBottomLatitude
				|| bottomLatitude > dataTopLatitude) {
			return false;
		}
		return true;
	}
