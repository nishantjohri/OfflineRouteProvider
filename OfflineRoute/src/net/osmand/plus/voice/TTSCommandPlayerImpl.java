...
public class TTSCommandPlayerImpl extends AbstractPrologCommandPlayer {
	...
	private static final int[] TTS_VOICE_VERSION = new int[] { 100, 101 }; // !!
																			// MUST
																			// BE
																			// SORTED
	...
	public void playCommands(CommandBuilder builder) {
		if (mTts != null) {
			final List<String> execute = builder.execute(); // list of strings,
															// the speech text,
															// play it
				...
				public void onInit(int status) {
					if (status != TextToSpeech.SUCCESS) {
						...
						case TextToSpeech.LANG_NOT_SUPPORTED:
							// maybe weird, but I didn't want to introduce
							// parameter in around 5 methods just to do
							// this if condition
							if (isSettingsActivity(act)) {
								Builder builder = createAlertDialog(
										...
										new IntentStarter(
												...
												Intent.ACTION_VIEW,
												Uri
														.parse("market://search?q=text to speech engine")),
										act);
								builder.show();
							}
							break;
