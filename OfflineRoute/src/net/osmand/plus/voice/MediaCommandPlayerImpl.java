...
/**
 * That class represents command player. It gets commands from input, analyze
 * what files should be played and play them using media player
 */
public class MediaCommandPlayerImpl extends AbstractPrologCommandPlayer {

	private static final String CONFIG_FILE = "_config.p";
	private static final int[] MEDIA_VOICE_VERSION = new int[] { 0 }; // MUST BE
																		// SORTED,
																		// list
																		// of
																		// supported
		...
		while (!filesToPlay.isEmpty() && playNext) {
			...
			if (f != null && voiceDir != null) {
				...
				if (exists) {
					...
					playNext = false;
					try {
						// Can't play sound file from zip it seams to be
						// impossible only unpack and play!!!
								...
								.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
									...
									public void onCompletion(MediaPlayer mp) {
										...
										while (!filesToPlay.isEmpty() && delay) {
											delay = filesToPlay.get(0)
													.startsWith(DELAY_CONST);
											if (delay) {
												String s = filesToPlay
														.remove(0)
														.substring(
																DELAY_CONST
																		.length());
