...
public class TextRenderer {
	...
	static class TextDrawInfo {
		...
		public void fillProperties(RenderingRuleSearchRequest render,
				float centerX, float centerY) {
			this.centerX = centerX;
			this.centerY = centerY
					+ render.getIntPropertyValue(render.ALL.R_TEXT_DY, 0);
			...
			textShadow = render.getIntPropertyValue(
					render.ALL.R_TEXT_HALO_RADIUS, 0);
			textWrap = render.getIntPropertyValue(render.ALL.R_TEXT_WRAP_WIDTH,
					0);
			bold = render.getIntPropertyValue(render.ALL.R_TEXT_BOLD, 0) > 0;
			minDistance = render.getIntPropertyValue(
					render.ALL.R_TEXT_MIN_DISTANCE, 0);
			if (render.isSpecified(render.ALL.R_TEXT_SHIELD)) {
				shieldRes = render
						.getStringPropertyValue(render.ALL.R_TEXT_SHIELD);
			}
			textOrder = render
					.getIntPropertyValue(render.ALL.R_TEXT_ORDER, 100);
		}
	}
	...
	boolean intersects(RectF tRect, float tRot, RectF sRect, float sRot) {
		...
		float dist = FloatMath.sqrt(sqr(tRect.centerX() - sRect.centerX())
				+ sqr(tRect.centerY() - sRect.centerY()));
		if (dist < 3) {
			return true;
		}
		...
		if (Math.abs(FloatMath.sin(tRot - sRot)) < 0.3) {
			...
			float diff = (float) (-Math.atan2(
					tRect.centerX() - sRect.centerX(), tRect.centerY()
							- sRect.centerY()) + Math.PI / 2);
			diff -= sRot;
			float left = sRect.centerX() + dist * FloatMath.cos(diff)
					- tRect.width() / 2;
			float top = sRect.centerY() - dist * FloatMath.sin(diff)
					- tRect.height() / 2;
			RectF nRect = new RectF(left, top, left + tRect.width(), top
					+ tRect.height());
	...
	void drawTestBox(Canvas cv, RectF r, float rot, String text) {
		...
		RectF rs = new RectF(-r.width() / 2, -r.height() / 2, r.width() / 2, r
				.height() / 2);
	...
	private boolean findTextIntersection(Canvas cv, RenderingContext rc,
			QuadTree<TextDrawInfo> boundIntersections, TextDrawInfo text) {
		// for test purposes
		...
		if (text.minDistance > 0) {
			...
			boundsSearch.inset(-rc.getDensityValue(Math.max(5.0f,
					text.minDistance)), -rc.getDensityValue(15));
			boundIntersections.queryInBox(boundsSearch, tempSearch);
			// drawTestBox(cv, &boundsSearch, text.pathRotate, paintIcon,
			// text.text, NULL/*paintText*/);
			for (int i = 0; i < tempSearch.size(); i++) {
				TextDrawInfo t = tempSearch.get(i);
				if (t.minDistance > 0
						&& t.text.equals(text.text)
						&& intersects(boundsSearch, text.pathRotate, t.bounds,
								t.pathRotate)) {
					return true;
				}
			}
	...
	private void drawTextOnCanvas(Canvas cv, String text, float centerX,
			float centerY, Paint paint, float textShadow) {
		if (textShadow > 0) {
	...
	public void drawTextOverCanvas(RenderingContext rc, Canvas cv,
		...
		r.inset(-100, -100);
		QuadTree<TextDrawInfo> nonIntersectedBounds = new QuadTree<TextDrawInfo>(
				r, 4, 0.6f);
		...
		for (int i = 0; i < size; i++) {
			...
			if (text.text != null && text.text.length() > 0) {
				...
				// possibly it will be used specifying english names after that
				// character
				if (d > 0) {
				...
				boolean intersects = findTextIntersection(cv, rc,
						nonIntersectedBounds, text);
				if (!intersects) {
					if (text.drawOnPath != null) {
						...
						if (text.shieldRes != null) {
							Bitmap ico = RenderingIcons.getIcon(context,
									text.shieldRes);
							if (ico != null) {
								cv.drawBitmap(ico, text.centerX
										- ico.getWidth() / 2 - 0.5f,
										text.centerY - ico.getHeight() / 2
	...
	private void drawWrappedText(Canvas cv, TextDrawInfo text, float textSize) {
		...
		if (text.text.length() > text.textWrap) {
			...
			while (pos < end) {
				...
				if (lastSpace == -1) {
					drawTextOnCanvas(cv, text.text.substring(start, pos),
							text.centerX, text.centerY + line * (textSize + 2),
			...
			drawTextOnCanvas(cv, text.text, text.centerX, text.centerY,
					paintText, text.textShadow);
		}
	}

	private void createTextDrawInfo(RenderingRuleSearchRequest render,
		...
		if (render.search(RenderingRulesStorage.TEXT_RULES)) {
			if (render.getIntPropertyValue(render.ALL.R_TEXT_SIZE) > 0) {
				...
				text.bounds.inset(-rc.getDensityValue(3), -rc
						.getDensityValue(10));
				boolean display = true;
				if (path != null) {
					text.drawOnPath = path;
					display = calculatePathToRotate(rc, text, points,
							render.getIntPropertyValue(
									render.ALL.R_TEXT_ON_PATH, 0) != 0);
				}
				if (text.drawOnPath == null) {
					...
					text.bounds.offset(text.centerX - text.bounds.width() / 2,
							text.centerY - text.bounds.height() / 2);
				}
				if (display) {
	...
	public void renderText(String name, RenderingRuleSearchRequest render,
			RenderingContext rc, TagValuePair pair, float xMid, float yMid,
			Path path, PointF[] points) {
		String ref = null;
		...
		if (ref != null && ref.trim().length() > 0) {
			createTextDrawInfo(render, rc, pair, xMid, yMid, path, points, ref,
					true);
		}

		if (name != null && name.trim().length() > 0) {
			createTextDrawInfo(render, rc, pair, xMid, yMid, path, points,
					name, false);
		}
	}

	boolean calculatePathToRotate(RenderingContext rc, TextDrawInfo p,
			PointF[] points, boolean drawOnPath) {
			...
			if (visibleRoadLength > 3 * textw) {
				...
				do {
					...
					if (endInd - startInd > 2
							&& visibleRoadLength - distances[startInd] > normalTextLen) {
						visibleRoadLength -= distances[startInd];
						startInd++;
						ch = true;
					}
					if (endInd - startInd > 2
							&& visibleRoadLength - distances[endInd - 2] > normalTextLen) {
