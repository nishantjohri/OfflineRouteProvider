...
public class OsmandRenderer {
	private static final Log log = OsmLogUtil.getLog(OsmandRenderer.class);
	...
	static enum ShadowRenderingMode {
		...
		// int shadowRenderingMode = 1; // classic shadow (the implementaton in
		// master)
		// int shadowRenderingMode = 2; // blur shadow (most CPU, but still
		// reasonable)
		// int shadowRenderingMode = 3; solid border (CPU use like classic
		// version or even smaller)
	...
	public Shader getShader(String resId) {
		...
		if (shaders.get(resId) == null) {
			...
			if (bmp != null) {
				Shader sh = new BitmapShader(bmp, TileMode.REPEAT,
						TileMode.REPEAT);
	 ...
	 */
	public void generateNewBitmapNative(RenderingContext rc,
			NativeOsmandLibrary library,
			NativeSearchResult searchResultHandler, Bitmap bmp,
			boolean useEnglishNames, RenderingRuleSearchRequest render,
			final List<IMapDownloaderCallback> notifyList, int defaultColor) {
		long now = System.currentTimeMillis();
		if (rc.width > 0 && rc.height > 0 && searchResultHandler != null) {
			// init rendering context
			rc.tileDivisor = (int) (1 << (31 - rc.zoom));
			rc.cosRotateTileSize = FloatMath.cos((float) Math
					.toRadians(rc.rotate))
					* TILE_SIZE;
			rc.sinRotateTileSize = FloatMath.sin((float) Math
					.toRadians(rc.rotate))
					* TILE_SIZE;
			...
			try {
				...
				String res = library.generateRendering(rc, searchResultHandler,
						bmp, useEnglishNames, render, defaultColor);
				rc.ended = true;
				notifyListeners(notifyList);
				long time = System.currentTimeMillis() - now;
				rc.renderingDebugInfo = String
						.format(
								"Rendering: %s ms  (%s text)\n"
										+ "(%s points, %s points inside, %s of %s objects visible)\n" + res,//$NON-NLS-1$
								time, rc.textRenderingTime, rc.pointCount,
	...
	public void generateNewBitmap(RenderingContext rc,
			List<BinaryMapDataObject> objects, Bitmap bmp,
			boolean useEnglishNames, RenderingRuleSearchRequest render,
			final List<IMapDownloaderCallback> notifyList, int defaultColor) {
		long now = System.currentTimeMillis();

		if (objects != null && !objects.isEmpty() && rc.width > 0
				&& rc.height > 0) {
			// init rendering context
			rc.tileDivisor = (int) (1 << (31 - rc.zoom));
			rc.cosRotateTileSize = FloatMath.cos((float) Math
			...
			// put in order map
			TIntObjectHashMap<TIntArrayList> orderMap = sortObjectsByProperOrder(
					rc, objects, render);
			...
			for (int k = 0; k < keys.length; k++) {
				if (!shadowDrawn && keys[k] >= rc.shadowLevelMin
						&& keys[k] <= rc.shadowLevelMax
			...
			rc.renderingDebugInfo = String
					.format(
							...
							time, time - beforeIconTextTime, rc.pointCount,
							rc.pointInsideCount, rc.visible, rc.allObjects);
	...
	private void notifyListenersWithDelay(final RenderingContext rc,
			final List<IMapDownloaderCallback> notifyList, final Handler h) {
	...
	private void drawIconsOverCanvas(RenderingContext rc, Canvas cv) {
		...
		for (IconDrawInfo icon : rc.iconsToDraw) {
			if (icon.resId != null) {
				...
				if (ico != null) {
					if (icon.y >= 0 && icon.y < rc.height && icon.x >= 0
							&& icon.x < rc.width) {
						int z = (((int) icon.x / skewConstant) + ((int) icon.y / skewConstant)
								* iconsW);
						int i = z / 32;
	...
	private TIntObjectHashMap<TIntArrayList> sortObjectsByProperOrder(
			RenderingContext rc, List<BinaryMapDataObject> objects,
		...
		if (render != null) {
			...
			for (int i = 0; i < sz; i++) {
				...
				if (o instanceof MultyPolygon) {
					...
					render.setTagValueZoomLayer(((MultyPolygon) o).getTag(),
							((MultyPolygon) o).getValue(), rc.zoom, layer);
					render.setIntFilter(render.ALL.R_ORDER_TYPE,
							MapRenderingTypes.POLYGON_TYPE);
					if (render.search(RenderingRulesStorage.ORDER_RULES)) {
						int order = render
								.getIntPropertyValue(render.ALL.R_ORDER);
						put(orderMap, order, sh, init);
						if (render.isSpecified(render.ALL.R_SHADOW_LEVEL)) {
							rc.shadowLevelMin = Math.min(rc.shadowLevelMin,
									order);
							rc.shadowLevelMax = Math.max(rc.shadowLevelMax,
									order);
					...
					for (int j = 0; j < o.getTypes().length; j++) {
						// put(orderMap,
						// BinaryMapDataObject.getOrder(o.getTypes()[j]), sh +
						// j, init);
						int wholeType = o.getTypes()[j];
						int mask = wholeType & 3;
						int layer = 0;
						if (mask != MapRenderingTypes.POINT_TYPE) {
							layer = MapRenderingTypes
									.getNegativeWayLayer(wholeType);
						}

						TagValuePair pair = o.getMapIndex().decodeType(
								MapRenderingTypes.getMainObjectType(wholeType),
								MapRenderingTypes.getObjectSubType(wholeType));
						if (pair != null) {
							render.setTagValueZoomLayer(pair.tag, pair.value,
									rc.zoom, layer);
							render.setIntFilter(render.ALL.R_ORDER_TYPE, mask);
							if (render
									.search(RenderingRulesStorage.ORDER_RULES)) {
								int order = render
										.getIntPropertyValue(render.ALL.R_ORDER);
								put(orderMap, order, sh + j, init);
								if (render
										.isSpecified(render.ALL.R_SHADOW_LEVEL)) {
									rc.shadowLevelMin = Math.min(
											rc.shadowLevelMin, order);
									rc.shadowLevelMax = Math.max(
											rc.shadowLevelMax, order);
									render
											.clearValue(render.ALL.R_SHADOW_LEVEL);
								}
							}
						}
	...
	protected void drawObj(BinaryMapDataObject obj,
			RenderingRuleSearchRequest render, Canvas canvas,
			RenderingContext rc, int l, boolean renderText,
		...
		if (obj instanceof MultyPolygon) {
			...
			if (t == MapRenderingTypes.POINT_TYPE && !drawOnlyShadow) {
				...
				if (t == MapRenderingTypes.MULTY_POLYGON_TYPE
						&& !(obj instanceof MultyPolygon)) {
					// log this situation
					return;
				}
			}
	...
	private PointF calcMultiPolygonPoint(MultyPolygon o, int i, int b,
			RenderingContext rc) {
		rc.pointCount++;
	...
	private void drawMultiPolygon(BinaryMapDataObject obj,
			RenderingRuleSearchRequest render, Canvas canvas,
			RenderingContext rc) {
		...
		for (int i = 0; i < ((MultyPolygon) obj).getBoundsCount(); i++) {
			...
			if (cnt > 0) {
				...
				if (name != null) {
					textRenderer.renderText(name, render, rc, new TagValuePair(
							tag, value), xText / cnt, yText / cnt, null, null);
				}
			}
		}
	...
	private void drawPolygon(BinaryMapDataObject obj,
			RenderingRuleSearchRequest render, Canvas canvas,
			RenderingContext rc, TagValuePair pair) {
		...
		if (path != null && len > 0) {
			...
			if (name != null) {
				textRenderer.renderText(name, render, rc, pair, xText / len,
						yText / len, null, null);
			}
		}
	}

	private boolean updatePaint(RenderingRuleSearchRequest req, Paint p,
			int ind, boolean area, RenderingContext rc) {
		...
		if (ind == 0) {
			...
			if (rc.shadowRenderingMode == 1) {
				int shadowColor = req
						.getIntPropertyValue(req.ALL.R_SHADOW_COLOR);
				if (shadowColor == 0) {
					shadowColor = rc.shadowRenderingColor;
				}
				int shadowLayer = req
						.getIntPropertyValue(req.ALL.R_SHADOW_RADIUS);
				if (shadowColor == 0) {
					shadowLayer = 0;
				}
	...
	private void drawPoint(BinaryMapDataObject obj,
			RenderingRuleSearchRequest render, Canvas canvas,
			RenderingContext rc, TagValuePair pair, boolean renderText) {
		if (render == null || pair == null) {
			return;
		}
		...
		if (name != null && name.trim().length() > 0) {
			textRenderer.renderText(name, render, rc, pair, ps.x, ps.y, null,
					null);
		}

	}

	private void drawPolylineShadow(Canvas canvas, RenderingContext rc,
			Path path, int shadowColor, int shadowRadius) {
		// blurred shadows
	...
	private void drawPolyline(BinaryMapDataObject obj,
			RenderingRuleSearchRequest render, Canvas canvas,
			RenderingContext rc, TagValuePair pair, int layer,
			boolean drawOnlyShadow) {
		if (render == null || pair == null) {
			return;
		}
		...
		if (rc.zoom >= 16
				&& "highway".equals(pair.tag) && MapRenderingTypes.isOneWayWay(obj.getHighwayAttributes())) { //$NON-NLS-1$
			oneway = true;
		}
		...
		if (!drawOnlyShadow && obj.getName() != null
		...
		if (path != null) {
			if (drawOnlyShadow) {
				int shadowColor = render
						.getIntPropertyValue(render.ALL.R_SHADOW_COLOR);
				int shadowRadius = render
						.getIntPropertyValue(render.ALL.R_SHADOW_RADIUS);
				if (shadowColor == 0) {
					shadowColor = rc.shadowRenderingColor;
				}
			...
			if (textPoints != null) {
				textRenderer.renderText(obj.getName(), render, rc, pair, xMid,
						yMid, path, textPoints);
			}
		}
	}
	...
	public static Paint[] getOneWayPaints() {
		if (oneWay == null) {
			PathEffect arrowDashEffect1 = new DashPathEffect(new float[] { 0,
					12, 10, 152 }, 0);
			PathEffect arrowDashEffect2 = new DashPathEffect(new float[] { 0,
					12, 9, 153 }, 1);
			PathEffect arrowDashEffect3 = new DashPathEffect(new float[] { 0,
					18, 2, 154 }, 1);
			PathEffect arrowDashEffect4 = new DashPathEffect(new float[] { 0,
					18, 1, 155 }, 1);
			oneWay = new Paint[4];
