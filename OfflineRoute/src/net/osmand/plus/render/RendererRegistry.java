...
public class RendererRegistry {
	...
	private final static Log log = OsmLogUtil.getLog(RendererRegistry.class);
	...
	public RendererRegistry() {
		...
		internalRenderers.put("high-contrast-roads",
				"high-contrast-roads.render.xml");
	}
	...
	private boolean hasRender(String name) {
		return externalRenderers.containsKey(name)
				|| internalRenderers.containsKey(name);
	}

	private RenderingRulesStorage getRenderer(String name,
			Set<String> loadedRenderers) {
		try {
	...
	private RenderingRulesStorage loadRenderer(String name,
			final Set<String> loadedRenderers) throws IOException, SAXException {
		InputStream is = null;
		if (externalRenderers.containsKey(name)) {
			...
			is = RenderingRulesStorage.class
					.getResourceAsStream(internalRenderers.get(name));
		} else {
		...
		main.parseRulesFromXmlInputStream(is,
				new RenderingRulesStorageResolver() {

					@Override
					public RenderingRulesStorage resolve(String name,
							RenderingRulesStorageResolver ref)
							throws SAXException {
						if (renderers.containsKey(name)) {
							return renderers.get(name);
						}
						...
						RenderingRulesStorage dep = getRenderer(name,
								loadedRenderers);
						if (dep == null) {
	...
	public void setCurrentSelectedRender(
			RenderingRulesStorage currentSelectedRender) {
