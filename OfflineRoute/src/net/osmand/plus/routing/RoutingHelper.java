...
public class RoutingHelper {
	...
	private static final org.apache.commons.logging.Log log = OsmLogUtil
			.getLog(RoutingHelper.class);
	...
	public RoutingHelper(OsmandSettings settings, Context context,
	...
	public void setFinalAndCurrentLocation(LatLon finalLocation,
			Location currentLocation) {
		setFinalAndCurrentLocation(finalLocation, currentLocation, null);
	}

	public synchronized void setFinalAndCurrentLocation(LatLon finalLocation,
			Location currentLocation, GPXRouteParams gpxRoute) {
	...
	public List<Location> getCurrentRoute() {
		return currentGPXRoute == null || currentGPXRoute.points.isEmpty() ? Collections
				.unmodifiableList(routeNodes)
				: Collections.unmodifiableList(currentGPXRoute.points);
	}
	...
	public boolean finishAtLocation(Location currentLocation) {
		...
		if (currentRoute > routeNodes.size() - 3
				&& currentLocation.distanceTo(lastPoint) < 60) {
			if (lastFixedLocation != null
					&& lastFixedLocation.distanceTo(lastPoint) < 60) {
	...
	public void setCurrentLocation(Location currentLocation) {
		...
		synchronized (this) {
			if (routeNodes.isEmpty() || routeNodes.size() <= currentRoute) {
				...
				float dist = currentLocation.distanceTo(routeNodes
						.get(currentRoute));
				while (currentRoute + 1 < routeNodes.size()) {
					float newDist = currentLocation.distanceTo(routeNodes
							.get(currentRoute + 1));
					boolean proccesed = false;
					if (newDist < dist) {
						if (newDist > 150) {
							// may be that check is really not needed ? only for
							// start position
							if (currentRoute > 0) {
								// check that we are not far from the route (if
								// we are on the route distance doesn't matter)
								float bearing = routeNodes
										.get(currentRoute - 1).bearingTo(
												routeNodes.get(currentRoute));
								float bearingMovement = currentLocation
										.bearingTo(routeNodes.get(currentRoute));
								float d = Math
										.abs(currentLocation
												.distanceTo(routeNodes
														.get(currentRoute))
												* FloatMath
														.sin((bearingMovement - bearing) * 3.14f / 180f));
								if (d > 50) {
									proccesed = true;
								}
							} else {
							...
							if (proccesed && log.isDebugEnabled()) {
								log
										.debug("Processed distance : " + newDist + " " + dist); //$NON-NLS-1$//$NON-NLS-2$
							}

						} else {
							// case if you are getting close to the next point
							// after turn
							// but you haven't turned before (could be checked
							// bearing)
							if (currentLocation.hasBearing()
									|| lastFixedLocation != null) {
								float bearingToPoint = currentLocation
										.bearingTo(routeNodes.get(currentRoute));
								float bearingBetweenPoints = routeNodes.get(
										currentRoute).bearingTo(
										routeNodes.get(currentRoute + 1));
								float bearing = currentLocation.hasBearing() ? currentLocation
										.getBearing()
										: lastFixedLocation
												.bearingTo(currentLocation);
								if (Math.abs(bearing - bearingToPoint) > Math
										.abs(bearing - bearingBetweenPoints)) {
									if (log.isDebugEnabled()) {
										log
												.debug("Processed point bearing : " + Math.abs(currentLocation.getBearing() - bearingToPoint) + " " //$NON-NLS-1$ //$NON-NLS-2$
														+ Math
																.abs(currentLocation
																		.getBearing()
				...
				if (currentRoute + 1 < routeNodes.size()) {
					float bearing = routeNodes.get(currentRoute).bearingTo(
							routeNodes.get(currentRoute + 1));
					float bearingMovement = currentLocation
							.bearingTo(routeNodes.get(currentRoute));
					// only 35 degrees for that case because it wrong catches
					// sharp turns
					if (Math.abs(bearing - bearingMovement) > 140
							&& Math.abs(bearing - bearingMovement) < 220) {
						if (log.isDebugEnabled()) {
							log
									.debug("Processed point movement bearing  : " + bearingMovement + " bearing " + bearing); //$NON-NLS-1$ //$NON-NLS-2$
						}
						updateCurrentRoute(currentRoute + 1);
					}
				}

				// 3.5 check that we already pass very sharp turn by missing one
				// point (so our turn is sharper than expected)
				// instead of that rule possible could be introduced another if
				// the dist < 5m mark the location as already passed
				if (currentRoute + 2 < routeNodes.size()) {
					float bearing = routeNodes.get(currentRoute + 1).bearingTo(
							routeNodes.get(currentRoute + 2));
					float bearingMovement = currentLocation
							.bearingTo(routeNodes.get(currentRoute + 1));
					// only 15 degrees for that case because it wrong catches
					// sharp turns
					if (Math.abs(bearing - bearingMovement) > 165
							&& Math.abs(bearing - bearingMovement) < 195) {
						if (log.isDebugEnabled()) {
							log
									.debug("Processed point movement bearing 2 : " + bearingMovement + " bearing " + bearing); //$NON-NLS-1$ //$NON-NLS-2$
						}
				...
				if (currentRoute > 0) {
					...
					float bearingMovement = currentLocation
							.bearingTo(routeNodes.get(currentRoute));
					float d = Math
							.abs(currentLocation.distanceTo(routeNodes
									.get(currentRoute))
									* FloatMath
											.sin((bearingMovement - bearing) * 3.14f / 180f));
					if (d > 50) {
						log
								.info("Recalculate route, because correlation  : " + d); //$NON-NLS-1$
						calculateRoute = true;
					}
				}
				...
				if (!calculateRoute) {
					float d = currentLocation.distanceTo(routeNodes
							.get(currentRoute));
					if (d > 80) {
						if (currentRoute > 0) {
							// possibly that case is not needed (often it is
							// covered by 4.)
							float f1 = currentLocation.distanceTo(routeNodes
									.get(currentRoute - 1))
									+ d;
							float c = routeNodes.get(currentRoute - 1)
									.distanceTo(routeNodes.get(currentRoute));
							if (c * 2 < d + f1) {
								log
										.info("Recalculate route, because too far from points : " + d + " " + f1 + " >> " + c); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
								calculateRoute = true;
							}
						} else {
							// that case is needed
							log
									.info("Recalculate route, because too far from start : " + d); //$NON-NLS-1$
							calculateRoute = true;
						}
					}
				}
				...
				// }
				// bearingRoute =
				// currentLocation.bearingTo(routeNodes.get(currentRoute));
				// if (Math.abs(bearing - bearingRoute) > 60f && 360 -
				// Math.abs(bearing - bearingRoute) > 60f) {
				...
				if (routeNodes.size() > 0) {
					if (currentLocation.hasBearing()
						...
						float bearingRoute;
						bearingRoute = currentLocation.bearingTo(routeNodes
								.get(currentRoute));
						if (Math.abs(bearing - bearingRoute) > 135f
								&& 360 - Math.abs(bearing - bearingRoute) > 135f) {
							float d = currentLocation.distanceTo(routeNodes
									.get(currentRoute));
							if (d > 50) {
		...
		lastFixedLocation = currentLocation;
		if (calculateRoute) {
			recalculateRouteInBackground(lastFixedLocation, finalLocation,
					currentGPXRoute);
		}
	}
	...
	private synchronized void setNewRoute(RouteCalculationResult res) {
		...
		if (isFollowingMode) {
			voiceRouter
					.newRouteIsCalculated(updateRoute, makeUturnWhenPossible);
		}
		uiHandler.post(new Runnable() {
			@Override
			public void run() {
	...
	public String getGeneralRouteInformation() {
		...
		int seconds = getLeftTime() - (minutes*60) - (hours * 60 * 60);
		return context.getString(R.string.route_general_information,
				OsmAndFormatter.getFormattedDistance(dist, context), hours,
				minutes, seconds);
	}
	...
	public RouteDirectionInfo getNextRouteDirectionInfo() {
		if (directionInfo != null
				&& currentDirectionInfo < directionInfo.size() - 1) {
	...
	public RouteDirectionInfo getNextNextRouteDirectionInfo() {
		if (directionInfo != null
				&& currentDirectionInfo < directionInfo.size() - 2) {
	...
	public List<RouteDirectionInfo> getRouteDirections() {
		if (directionInfo != null
				&& currentDirectionInfo < directionInfo.size()) {
			if (currentDirectionInfo == 0) {
				return directionInfo;
			}
			if (currentDirectionInfo < directionInfo.size() - 1) {
				return directionInfo.subList(currentDirectionInfo + 1,
						directionInfo.size());
			}
		}
	...
	public int getDistanceToNextRouteDirection() {
		if (directionInfo != null
			...
			int dist = listDistance[currentRoute];
			if (currentDirectionInfo < directionInfo.size() - 1) {
				dist -= listDistance[directionInfo
						.get(currentDirectionInfo + 1).routePointOffset];
			}
			if (lastFixedLocation != null) {
				dist += lastFixedLocation.distanceTo(routeNodes
						.get(currentRoute));
			}
	...
	public synchronized int getLeftTime() {
		if (directionInfo != null
			...
			if (e > 0) {
				int passedDist = listDistance[directionInfo
						.get(currentDirectionInfo).routePointOffset]
						- listDistance[currentRoute];
				int wholeDist = listDistance[directionInfo
						.get(currentDirectionInfo).routePointOffset];
				if (currentDirectionInfo < directionInfo.size() - 1) {
					wholeDist -= listDistance[directionInfo
							.get(currentDirectionInfo + 1).routePointOffset];
				}
				if (wholeDist > 0) {
					t = (int) (t + ((float) e)
							* (1 - (float) passedDist / (float) wholeDist));
				}
			}
			return t;
		}
	...
	private void recalculateRouteInBackground(final Location start,
			final LatLon end, final GPXRouteParams gpxRoute) {
		...
		if (serviceToUse == RouteService.OSMAND
				&& !settings.USE_OSMAND_ROUTING_SERVICE_ALWAYS.get()) {
			double distance = MapUtils.getDistance(end, start.getLatitude(),
					start.getLongitude());
			if (distance > DISTANCE_TO_USE_OSMAND_ROUTER) {
				// display 'temporarily switched to CloudMade' message only once
				// per error wait period and not for GPX routes
				if (evalWaitInterval == 3000
						&& settings.FOLLOW_THE_GPX_ROUTE.get() == null) {
					showMessage(context
							.getString(R.string.osmand_routing_experimental),
							Toast.LENGTH_LONG);
			...
			if (System.currentTimeMillis() - lastTimeEvaluatedRoute > evalWaitInterval) {
				...
				synchronized (this) {
					currentRunningJob = new Thread(new Runnable() {
						@Override
						public void run() {
							RouteCalculationResult res = provider
									.calculateRouteImpl(start, end, mode,
											service, context, gpxRoute,
											fastRouteMode);
							...
							if (res.isCalculated()) {
								...
								int l = dist != null && dist.length > 0 ? dist[0]
										: 0;
								showMessage(context
										.getString(R.string.new_route_calculated_dist)
										+ ": " + OsmAndFormatter.getFormattedDistance(l, context)); //$NON-NLS-1$
							} else if (service != RouteService.OSMAND
									&& !settings
											.isInternetConnectionAvailable()) {
								showMessage(
										context
												.getString(R.string.error_calculating_route)
								...
								if (res.getErrorMessage() != null) {
									...
									showMessage(
											context
													.getString(R.string.error_calculating_route_occured),
											Toast.LENGTH_LONG);
								} else {
									showMessage(
											context
													.getString(R.string.empty_route_calculated),
											Toast.LENGTH_LONG);
								}
							}
	...
	public synchronized void fillLocationsToShow(double topLatitude,
		...
		if (lastFixedLocation != null) {
			if (leftLongitude <= lastFixedLocation.getLongitude()
					&& lastFixedLocation.getLongitude() <= rightLongitude
					&& bottomLatitude <= lastFixedLocation.getLatitude()
					&& lastFixedLocation.getLatitude() <= topLatitude) {
		...
		for (int i = currentRoute; i < routeNodes.size(); i++) {
			...
			if (leftLongitude <= ls.getLongitude()
					&& ls.getLongitude() <= rightLongitude
					&& bottomLatitude <= ls.getLatitude()
				...
				// do not continue make method more efficient (because it calls
				// in UI thread)
	...
	public GPXFile generateGPXFileWithRoute() {
		return provider.createOsmandRouterGPX(currentRoute, routeNodes,
	...
	public static class TurnType {
		...
		public static String[] predefinedTypes = new String[] { C, TL, TSLL,
				TSHL, TR, TSLR, TSHR, TU, TRU };
