		...
		if (!isRouteAnimating()) {
			routeAnimation = new Thread() {
				...
				public void run() {
					...
					Location current = directions.isEmpty() ? null
							: new Location(directions.remove(0));
					Location prev = null;
				...
				private float metersToGoInFiveSteps(
					...
					return directions.isEmpty() ? 20.0f : Math.max(20.0f,
							current.distanceTo(directions.get(0)) / 2);
				};
			};
