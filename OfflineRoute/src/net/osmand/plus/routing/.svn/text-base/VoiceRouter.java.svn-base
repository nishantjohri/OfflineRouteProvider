...
public class VoiceRouter {
	// 0 - unknown, 1 - notify to prepare, 2 - notify to turn in , 3 - notify to
	// turn
	...
	// default speed to have comfortable announcements (if actual speed is
	// higher than it would be problem)
	// Speed in m/s
	...
	private boolean isDistanceLess(float currentSpeed, double dist,
			double etalon) {
	...
	protected void updateStatus(Location currentLocation,
		...
		if (statusNotPassed(STATUS_TURN)
				&& isDistanceLess(speed, dist, TURN_DISTANCE)) {
		...
		} else if (statusNotPassed(STATUS_200_TURN)
				&& isDistanceLess(speed, dist, TURN_IN_DISTANCE)) {
		...
		} else if (statusNotPassed(STATUS_800_PREPARE)
				&& isDistanceLess(speed, dist, PREPARE_DISTANCE)) {
		...
		} else if (statusNotPassed(STATUS_3000_PREPARE)
				&& isDistanceLess(speed, dist, PREPARE_LONG_DISTANCE_END)) {
		...
		if (play != null) {
			...
			if (tParam != null) {
				...
				play.prepareRoundAbout(dist).play();
					...
					|| next.turnType.getValue().equals(TurnType.TRU)) {
				play.prepareMakeUT(dist).play();
			}
		}
	}

	private void playMakeTurnInShortDistance(RouteDirectionInfo next, int dist,
			RouteDirectionInfo pronounceNextNext) {
		CommandBuilder play = getNewCommandPlayerToPlay();
		if (play != null) {
			String tParam = getTurnType(next.turnType);
			boolean isPlay = true;
			if (tParam != null) {
				...
				play.roundAbout(dist, next.turnType.getTurnAngle(),
						next.turnType.getExitOut());
			} else if (next.turnType.getValue().equals(TurnType.TU)
					|| next.turnType.getValue().equals(TurnType.TRU)) {
				play.makeUT(dist);
			} else {
	...
	private void playMakeTurnRightNow(RouteDirectionInfo next,
			RouteDirectionInfo nextNext) {
		CommandBuilder play = getNewCommandPlayerToPlay();
		if (play != null) {
			String tParam = getTurnType(next.turnType);
			boolean isplay = true;
			if (tParam != null) {
				...
				play.roundAbout(next.turnType.getTurnAngle(), next.turnType
						.getExitOut());
			} else if (next.turnType.getValue().equals(TurnType.TU)
					|| next.turnType.getValue().equals(TurnType.TRU)) {
				play.makeUT();
			...
			if (nextNext != null) {
				...
				if (t2Param != null) {
					...
					play.roundAbout(next.distance, nextNext.turnType
							.getTurnAngle(), nextNext.turnType.getExitOut());
	...
	public void newRouteIsCalculated(boolean updateRoute,
		...
		if (play != null) {
			if (updateRoute) {
				// suppress "route recalculated" prompt while
				// makeUturnWhenPossible is active
				if (makeUturnWhenPossible == false) {
					// suppress "route recaluated" prompt for 60sec
					if (System.currentTimeMillis()
							- lastTimeRouteRecalcAnnounced > 60000) {
						// suppress "route recaluated" prompt for GPX-rotuing,
						// it makes no sense
						if (router.getCurrentGPXRoute() == null) {
							play.routeRecalculated(router.getLeftDistance())
									.play();
							lastTimeRouteRecalcAnnounced = System
									.currentTimeMillis();
						}
					}
				}
			...
			pendingCommand = new VoiceCommandPending(
					updateRoute ? VoiceCommandPending.ROUTE_RECALCULATED
