...
public class NameFinderPoiFilter extends PoiFilter {
	...
	private static final Log log = OsmLogUtil.getLog(NameFinderPoiFilter.class);
	...
	public NameFinderPoiFilter(OsmandApplication application) {
		...
		this.distanceToSearchValues = new double[] { 1, 2, 5, 10, 20, 30, 100,
				250 };
	...
	protected List<Amenity> searchAmenities(double lat, double lon,
			...
			double rightLongitude, ResultMatcher<Amenity> matcher) {
		searchedAmenities.clear();

		String viewbox = "viewboxlbrt=" + ((float) leftLongitude) + ","
				+ ((float) bottomLatitude) + "," + ((float) rightLongitude)
				+ "," + ((float) topLatitude);
		try {
			lastError = "";
			String urlq = "http://nominatim.openstreetmap.org/search/"
					+ URLEncoder.encode(query)
					+ "?format=xml&addressdetails=1&limit=" + LIMIT
					+ "&bounded=1&" + viewbox;
			log.info(urlq);
				...
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (parser.getName().equals("place")) { //$NON-NLS-1$
						...
						if (namedDepth == 1) {
							try {
								a = new Amenity();
								a.setLocation(Double.parseDouble(parser
										.getAttributeValue("", "lat")), //$NON-NLS-1$//$NON-NLS-2$
										Double.parseDouble(parser
												.getAttributeValue("", "lon"))); //$NON-NLS-1$//$NON-NLS-2$
								a.setId(Long.parseLong(parser
										.getAttributeValue("", "place_id"))); //$NON-NLS-1$ //$NON-NLS-2$
								...
								a.setSubType(parser.getAttributeValue(
							...
							&& parser.getName().equals(a.getSubType())) {
