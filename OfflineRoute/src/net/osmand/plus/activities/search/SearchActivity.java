...
public class SearchActivity extends TabActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		host.addTab(host.newTabSpec("Search_POI").setIndicator(
				getTabIndicator(R.drawable.tab_search_poi_icon)).setContent(
				new Intent(this, SearchPoiFilterActivity.class))); //$NON-NLS-1$

		addressSpec = host.newTabSpec("Search_Address").setIndicator(
				getTabIndicator(R.drawable.tab_search_address_icon));
		...
		// TabSpec transportTab =
		// host.newTabSpec("Search_Transport").setIndicator(getTabIndicator(R.drawable.tab_search_transport_icon)).setContent(createIntent(SearchTransportActivity.class));
							...
							if (position == POSITION_CURRENT_LOCATION) {
								...
								if (position == POSITION_LAST_MAP_VIEW) {
									OsmandSettings settings = OsmandSettings
											.getOsmandSettings(SearchActivity.this);
									updateSearchPoint(
									...
									intent
											.putExtra(
													FavouritesListActivity.SELECT_FAVORITE_POINT_INTENT_KEY,
													(Serializable) null);
									startActivityForResult(intent,
											REQUEST_FAVORITE_SELECT);
									...
									intent
											.putExtra(
													SearchAddressActivity.SELECT_ADDRESS_POINT_INTENT_KEY,
													(String) null);
									startActivityForResult(intent,
											REQUEST_ADDRESS_SELECT);
									spinner.setSelection(0);
								}
							}
						}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_FAVORITE_SELECT
				&& resultCode == FavouritesListActivity.SELECT_FAVORITE_POINT_RESULT_OK) {
			FavouritePoint p = (FavouritePoint) data
					.getSerializableExtra(FavouritesListActivity.SELECT_FAVORITE_POINT_INTENT_KEY);
			if (p != null) {
				LatLon latLon = new LatLon(p.getLatitude(), p.getLongitude());
				updateSearchPoint(latLon, p.getName(), false);
			}
		} else if (requestCode == REQUEST_ADDRESS_SELECT
				&& resultCode == SearchAddressActivity.SELECT_ADDRESS_POINT_RESULT_OK) {
			String name = data
					.getStringExtra(SearchAddressActivity.SELECT_ADDRESS_POINT_INTENT_KEY);
			LatLon latLon = new LatLon(data.getDoubleExtra(
					SearchAddressActivity.SELECT_ADDRESS_POINT_LAT, 0), data
					.getDoubleExtra(
							SearchAddressActivity.SELECT_ADDRESS_POINT_LON, 0));
			if (name != null) {
	...
	public void startSearchCurrentLocation() {
		if (locationListener == null) {
			locationListener = new LocationListener() {
				...
				public void onLocationChanged(Location location) {
					if (location != null) {
						updateSearchPoint(
								new LatLon(location.getLatitude(), location
										.getLongitude()),
								getString(R.string.search_position_current_location_found),
								false);
		...
		updateSearchPoint(null,
				getString(R.string.search_position_current_location_search),
				false);
	}
	...
	protected void onResume() {
		...
		if (searchPoint == null) {
			LatLon last = OsmandSettings.getOsmandSettings(this)
					.getLastKnownMapLocation();
			if (!Algoritms.objectEquals(reqSearchPoint, last)) {
				reqSearchPoint = last;
				updateSearchPoint(last,
						getString(R.string.search_position_fixed), true);
			}
		}
	}
	...
	private String formatLatLon(LatLon searchPoint) {
		return new Formatter(Locale.US).format(" %.2f;%.2f",
	...
	public void setAddressSpecContent() {
		if (searchOnLine) {
			addressSpec
					.setContent(createIntent(SearchAddressOnlineActivity.class));
		} else {
