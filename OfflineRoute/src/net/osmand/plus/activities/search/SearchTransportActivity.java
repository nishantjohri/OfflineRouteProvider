 ...
 * 
 */
public class SearchTransportActivity extends TrackedListActivity implements
		SearchActivityChild {
	...
	public void onCreate(Bundle icicle) {
		...
		stopsAdapter = new TransportStopAdapter(
				new ArrayList<RouteInfoLocation>());
		...
		intermediateListAdapater = new TransportRouteAdapter(
				TransportRouteHelper.getInstance().getRoute());
	...
	protected void onResume() {
		...
		if (startPoint == null) {
			startPoint = OsmandSettings.getOsmandSettings(this)
					.getLastKnownMapLocation();
		}

		LatLon pointToNavigate = settings.getPointToNavigate();
		if (!Algoritms.objectEquals(pointToNavigate, this.destinationLocation)
				|| !Algoritms.objectEquals(startPoint,
						this.lastKnownMapLocation)) {
	...
	public void searchTransport() {
		...
		if (!routeCalculated && getLocationToStart() != null) {
			...
			List<TransportIndexRepository> rs = ((OsmandApplication) getApplication())
					.getResourceManager().searchTransportRepositories(
				...
				synchronized (this) {
					...
					thread = new Thread(new Runnable() {
						...
						public void run() {
							...
							List<RouteInfoLocation> res = repo
									.searchTransportRouteStops(locationToStart
											.getLatitude(), locationToStart
	...
	public String getInformation(RouteInfoLocation route,
			List<TransportStop> stops, int position, boolean part) {
		...
		for (TransportStop s : stops) {
			...
			if (ind > stInd && ind <= eInd) {
				dist += MapUtils.getDistance(stops.get(ind - 1).getLocation(),
						s.getLocation());
			}
			ind++;
		}
		text
				.append(getString(R.string.transport_route_distance))
				.append(" ").append(OsmAndFormatter.getFormattedDistance((int) dist, this)); //$NON-NLS-1$/
		if (!part) {
			text
					.append(", ").append(getString(R.string.transport_stops_to_pass)).append(" ").append(eInd - stInd); //$NON-NLS-1$ //$NON-NLS-2$
			LatLon endStop = getEndStop(position - 1);
			if (endStop != null) {
				String before = OsmAndFormatter.getFormattedDistance(
						(int) MapUtils.getDistance(endStop, route.getStart()
								.getLocation()), SearchTransportActivity.this);
				text
						.append(", ").append(getString(R.string.transport_to_go_before)).append(" ").append(before); //$NON-NLS-2$//$NON-NLS-1$
			}

			LatLon stStop = getStartStop(position + 1);
			if (stStop != null) {
				String after = OsmAndFormatter.getFormattedDistance(
						(int) MapUtils.getDistance(stStop, route.getStop()
								.getLocation()), SearchTransportActivity.this);
				text
						.append(", ").append(getString(R.string.transport_to_go_after)).append(" ").append(after); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
	...
	public void onListItemClick(ListView parent, View v, int position, long id) {
		final RouteInfoLocation item = ((TransportStopAdapter) getListAdapter())
				.getItem(position);
		Builder builder = new AlertDialog.Builder(this);
		List<String> items = new ArrayList<String>();
		final List<TransportStop> stops = item.getDirection() ? item.getRoute()
				.getForwardStops() : item.getRoute().getBackwardStops();
		LatLon locationToGo = getLocationToGo();
		LatLon locationToStart = getLocationToStart();
		builder
				.setTitle(getString(R.string.transport_stop_to_go_out)
		...
		for (TransportStop st : stops) {
			...
			if (locationToGo != null) {
				...
				n.append(
						OsmAndFormatter.getFormattedDistance((int) MapUtils
								.getDistance(locationToGo, st.getLocation()),
								SearchTransportActivity.this)).append("]"); //$NON-NLS-1$ 
			} else if (locationToStart != null) {
				n
						.append("[").append(OsmAndFormatter.getFormattedDistance((int) MapUtils.getDistance(locationToStart, st.getLocation()), SearchTransportActivity.this)).append("] - "); //$NON-NLS-1$ //$NON-NLS-2$
				n.append(name);
			} else {
				n.append(name);
			}
	...
	public void showContextMenuOnStop(final TransportStop stop,
			final RouteInfoLocation route, final int stopInd) {
		Builder b = new AlertDialog.Builder(this);
		b.setItems(new String[] { getString(R.string.transport_finish_search),
				...
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (which == 0) {
							intermediateListAdapater.insert(route,
									currentRouteCalculation);
	...
	public void showContextMenuOnRoute(final RouteInfoLocation route,
			final int routeInd) {
		Builder b = new AlertDialog.Builder(this);
		List<TransportStop> stops = route.getDirection() ? route.getRoute()
				.getForwardStops() : route.getRoute().getBackwardStops();
		...
		b.setNeutralButton(getString(R.string.transport_search_before),
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
		...
		b.setNegativeButton(getString(R.string.transport_search_after),
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						int toInsert = routeInd;
	...
	class TransportStopAdapter extends ArrayAdapter<RouteInfoLocation> {
		...
		TransportStopAdapter(List<RouteInfoLocation> list) {
			super(SearchTransportActivity.this,
					R.layout.search_transport_list_item, list);
			model = list;
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (row == null) {
				...
				row = inflater.inflate(R.layout.search_transport_list_item,
						parent, false);
			}
			LatLon locationToGo = getLocationToGo();
			LatLon locationToStart = getLocationToStart();
			...
			if (locationToGo != null) {
				labelW.append(OsmAndFormatter.getFormattedDistance(stop
						.getDistToLocation(), SearchTransportActivity.this));
			} else {
			...
			int dist = locationToStart == null ? 0
					: (int) (MapUtils.getDistance(
							stop.getStart().getLocation(), locationToStart));
			distanceLabel
					.setText(" " + OsmAndFormatter.getFormattedDistance(dist, SearchTransportActivity.this)); //$NON-NLS-1$
	...
	class TransportRouteAdapter extends ArrayAdapter<RouteInfoLocation> {
		TransportRouteAdapter(List<RouteInfoLocation> list) {
			super(SearchTransportActivity.this,
					R.layout.search_transport_route_item, list);
		}
		...
		public View getView(final int position, View convertView,
			...
			if (info == null) {
				...
				if (st != null && end != null) {
					...
					text.setText(MessageFormat.format(
							...
							OsmAndFormatter.getFormattedDistance(dist,
									SearchTransportActivity.this)));
				} else {
					text
							.setText(getString(R.string.transport_searching_transport));
				}
				text.setTextSize(21);
			...
			if (row == null || row instanceof TextView) {
				LayoutInflater inflater = getLayoutInflater();
				row = inflater.inflate(R.layout.search_transport_route_item,
						parent, false);
			}
			...
			boolean en = settings.usingEnglishNames();
			labelW
					.append(" : ").append(info.getStart().getName(en)).append(" - ").append(info.getStop().getName(en)); //$NON-NLS-1$ //$NON-NLS-2$
			// additional information if route is calculated
			if (currentRouteLocation == -1) {
				labelW.append(" ("); //$NON-NLS-1$
				labelW
						.append(info.getStopNumbers())
						.append(" ").append(getString(R.string.transport_stops)).append(", "); //$NON-NLS-1$ //$NON-NLS-2$
				int startDist = (int) MapUtils
						.getDistance(getEndStop(position - 1), info.getStart()
								.getLocation());
				labelW
						.append(getString(R.string.transport_to_go_before))
						.append(" ").append(OsmAndFormatter.getFormattedDistance(startDist, SearchTransportActivity.this)); //$NON-NLS-1$
				if (position == getCount() - 1) {
					LatLon stop = getStartStop(position + 1);
					if (stop != null) {
						int endDist = (int) MapUtils.getDistance(stop, info
								.getStop().getLocation());
						labelW
								.append(", ").append(getString(R.string.transport_to_go_after)).append(" ").append(OsmAndFormatter.getFormattedDistance(endDist, SearchTransportActivity.this)); //$NON-NLS-1$ //$NON-NLS-2$
					}
				}
