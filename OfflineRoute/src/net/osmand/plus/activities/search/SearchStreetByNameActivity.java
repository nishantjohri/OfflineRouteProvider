...
public class SearchStreetByNameActivity extends
		...
		return new AsyncTask<Object, Street, List<Street>>() {
			...
			protected List<Street> doInBackground(Object... params) {
				...
				if (region != null) {
					...
					if (postcode == null) {
						city = region.getCityById(settings
								.getLastSearchedCity());
						if (city == null) {
							return null;
						}
					}
					region.preloadStreets(postcode == null ? city : postcode,
							new ResultMatcher<Street>() {
								@Override
	...
	protected void filterLoop(String query, List<Street> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (emptyQuery
					|| CollatorStringMatcher.cmatches(collator, getText(obj),
