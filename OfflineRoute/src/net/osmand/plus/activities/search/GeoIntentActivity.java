...
public class GeoIntentActivity extends OsmandListActivity {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		settings.setMapLocationToShow(item.getLocation().getLatitude(), item
				.getLocation().getLongitude(), settings.getLastKnownMapZoom(),
		...
		public Collection<MapObject> execute() {
			...
			for (RegionAddressRepository rar : countriesToSearch) {
				...
				for (String maybeCity : elements) {
					citiesFound.addAll(rar.fillWithSuggestedCities(maybeCity,
							null, null));
				}
			...
			if (citiesForRegion.isEmpty()) {
				for (RegionAddressRepository rar : countriesToSearch) {
					List<MapObject> allcities = rar.fillWithSuggestedCities("",
							null, location);
				...
				for (RegionAddressRepository rar : citiesForRegion.keySet()) {
					findStreetsForCities(streetsForCity, rar, citiesForRegion
							.get(rar));
				}
			}
			...
			if (latIndex > 0) {
				try {
					...
					runOnUiThread(new Runnable() {
						@Override
						public void run() {
							Toast.makeText(GeoIntentActivity.this, getString(
									R.string.search_offline_geo_error, geo),
									Toast.LENGTH_LONG);
						}
					});
				}
