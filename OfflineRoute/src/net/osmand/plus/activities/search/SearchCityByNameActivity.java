...
public class SearchCityByNameActivity extends
		SearchByNameAbstractActivity<MapObject> {
		...
		return new AsyncTask<Object, MapObject, List<MapObject>>() {
			...
			protected void onPostExecute(List<MapObject> result) {
				((TextView) findViewById(R.id.Label))
						.setText(R.string.incremental_search_city);
			...
			protected void onPreExecute() {
				((TextView) findViewById(R.id.Label))
						.setText(R.string.loading_cities);
	...
	protected void filterLoop(String query, List<MapObject> list) {
		if (!initializeTaskIsFinished() || query.length() <= 2) {
			...
			region.fillWithSuggestedCities(query,
	...
	public String getText(MapObject obj) {
		...
		if (getFilter().length() > 2 && locationToSearch != null && l != null) {
			...
			if (obj instanceof City) {
				name += " ["
						+ OsmAndFormatter.toPublicString(
								((City) obj).getType(), this) + "]";
			}
			return name
					+ " - " + //$NON-NLS-1$
					OsmAndFormatter.getFormattedDistance((int) MapUtils
							.getDistance(l, locationToSearch), this);
		} else {
	...
	public void itemSelected(MapObject obj) {
		if (obj instanceof City) {
			settings.setLastSearchedCity(obj.getId(), obj.getName(region
					.useEnglishNames()), obj.getLocation());
			if (region.getCityById(obj.getId()) == null) {
				region.addCityToPreloadedList((City) obj);
			}
		} else if (obj instanceof PostCode) {
			settings.setLastSearchedPostcode(obj.getName(region
					.useEnglishNames()), obj.getLocation());
		}
		finish();
