...
public class SearchHistoryHelper {
	...
	private void updateModelAt(HistoryEntry model, HistoryItemDBHelper helper,
			int i) {
		if (i == -1) {
			if (helper.add(model)) {
				loadedEntries.add(0, model);
				if (loadedEntries.size() > HISTORY_LIMIT) {
					if (helper.remove(loadedEntries
							.get(loadedEntries.size() - 1))) {
	...
	public HistoryEntry addNewItemToHistory(double lat, double lon,
	...
	private static class HistoryItemDBHelper extends SQLiteOpenHelper {
		...
		private static final String HISTORY_TABLE_CREATE = "CREATE TABLE " + HISTORY_TABLE_NAME + " (" + //$NON-NLS-1$ //$NON-NLS-2$
				HISTORY_COL_NAME
				+ " TEXT, " + HISTORY_COL_TIME + " long, " + HISTORY_COL_TYPE + " TEXT, " + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ 
		...
		public boolean remove(HistoryEntry e) {
			...
			if (db != null) {
				db
						.execSQL(
								"DELETE FROM " + HISTORY_TABLE_NAME + " WHERE " + HISTORY_COL_NAME + " = ?", new Object[] { e.getName() }); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				return true;
			}
			return false;
		}
				...
				Cursor query = db
						.rawQuery(
								"SELECT " + HISTORY_COL_NAME + ", " + HISTORY_COL_LAT + "," + HISTORY_COL_LON + " FROM " + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ 
										HISTORY_TABLE_NAME
										+ " ORDER BY " + HISTORY_COL_TIME + " DESC", null); //$NON-NLS-1$//$NON-NLS-2$
				if (query.moveToFirst()) {
					do {
						HistoryEntry e = new HistoryEntry(query.getDouble(1),
								query.getDouble(2), query.getString(0));
						entries.add(e);
