...
public class SearchBuildingByNameActivity extends
		SearchByNameAbstractActivity<Building> {
		...
		return new AsyncTask<Object, Void, List<Building>>() {
			...
			protected void onPostExecute(List<Building> result) {
				((TextView) findViewById(R.id.Label))
						.setText(R.string.incremental_search_building);
			...
			protected void onPreExecute() {
				((TextView) findViewById(R.id.Label))
						.setText(R.string.loading_streets_buildings);
			...
			protected List<Building> doInBackground(Object... params) {
				...
				if (region != null) {
					postcode = region.getPostcode(settings
							.getLastSearchedPostcode());
					city = region.getCityById(settings.getLastSearchedCity());
					if (postcode != null) {
						street = region.getStreetByName(postcode, settings
								.getLastSearchedStreet());
					} else if (city != null) {
						street = region.getStreetByName(city, settings
								.getLastSearchedStreet());
					}
				}
				if (street != null) {
					// preload here to avoid concurrent modification
					region.preloadBuildings(street,
							new ResultMatcher<Building>() {
								@Override
	...
	public boolean filterObject(Building obj, String filter) {
		if (postcode != null
				&& !postcode.getName().equalsIgnoreCase(obj.getPostcode())) {
			return false;
		}
