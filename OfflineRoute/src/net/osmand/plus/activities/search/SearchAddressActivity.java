...
public class SearchAddressActivity extends Activity {
	...
	private String region = "Singapore";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		osmandSettings = OsmandSettings
				.getOsmandSettings(SearchAddressActivity.this);
		attachListeners();
	}
	...
	private Intent createIntent(Class<?> cl) {
		...
		if (location != null) {
			newIntent.putExtra(SearchActivity.SEARCH_LAT, location
					.getLatitude());
			newIntent.putExtra(SearchActivity.SEARCH_LON, location
					.getLongitude());
		}
		return newIntent;
	}
	...
	private void attachListeners() {
		...
		findViewById(R.id.ResetBuilding).setOnClickListener(
		...
		findViewById(R.id.ResetCountry).setOnClickListener(
				new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						// region = null;
	...
	public void showOnMap(boolean navigateTo) {
		...
		if (!Algoritms.isEmpty(street2) && !Algoritms.isEmpty(street)) {
			...
			objectName = street;
			historyName = MessageFormat.format(
					getString(R.string.search_history_int_streets), street,
					street2, cityName);
			zoom = 16;
			...
			historyName = MessageFormat.format(
					getString(R.string.search_history_building), building,
					street, cityName);
			zoom = 16;
		} else if (!Algoritms.isEmpty(street)) {
			String cityName = postcode != null ? postcode : city;
			objectName = street;
			historyName = MessageFormat
					.format(getString(R.string.search_history_street), street,
							cityName);
			zoom = 15;
		} else if (!Algoritms.isEmpty(city)) {
			historyName = MessageFormat.format(
					getString(R.string.search_history_city), city);
			objectName = city;
			zoom = 13;
		}
		if (selectAddressMode) {
			...
			intent
					.putExtra(SELECT_ADDRESS_POINT_LAT, searchPoint
							.getLatitude());
			intent.putExtra(SELECT_ADDRESS_POINT_LON, searchPoint
					.getLongitude());
			setResult(SELECT_ADDRESS_POINT_RESULT_OK, intent);
			finish();
		} else {
			if (navigateTo) {
				osmandSettings.setPointToNavigate(searchPoint.getLatitude(),
						searchPoint.getLongitude(), historyName);
			} else {
				osmandSettings.setMapLocationToShow(searchPoint.getLatitude(),
						searchPoint.getLongitude(), zoom, historyName);
			}
	...
	protected void updateBuildingSection() {
		if (radioBuilding) {
			((TextView) findViewById(R.id.BuildingText))
					.setText(R.string.search_address_building);
			if (Algoritms.isEmpty(building)) {
				((TextView) findViewById(R.id.BuildingButton))
						.setText(R.string.choose_building);
			} else {
				((TextView) findViewById(R.id.BuildingButton))
						.setText(building);
			}
			...
			((TextView) findViewById(R.id.BuildingText))
					.setText(R.string.search_address_street);
			if (Algoritms.isEmpty(street2)) {
				((TextView) findViewById(R.id.BuildingButton))
						.setText(R.string.choose_intersected_street);
			} else {
				((TextView) findViewById(R.id.BuildingButton)).setText(street2);
			}
		}
		findViewById(R.id.ResetBuilding).setEnabled(
				!Algoritms.isEmpty(street2) || !Algoritms.isEmpty(building));
	}
	...
	protected void updateUI() {
		...
		findViewById(R.id.ResetCity).setEnabled(
				!Algoritms.isEmpty(city) || !Algoritms.isEmpty(postcode));
		...
		streetButton.setEnabled(!Algoritms.isEmpty(city)
				|| !Algoritms.isEmpty(postcode));

		buildingButton.setEnabled(!Algoritms.isEmpty(street));
		((RadioGroup) findViewById(R.id.RadioGroup)).setVisibility(Algoritms
				.isEmpty(street) ? View.GONE : View.VISIBLE);

		if (radioBuilding) {
			((RadioButton) findViewById(R.id.RadioBuilding)).setChecked(true);
		} else {
			((RadioButton) findViewById(R.id.RadioIntersStreet))
					.setChecked(true);
		}
		updateBuildingSection();
			...
			if (!Algoritms.isEmpty(postcode) || !Algoritms.isEmpty(city)) {
				...
				if (!Algoritms.isEmpty(street)) {
					String str = osmandSettings
							.getLastSearchedIntersectedStreet();
	...
	protected void onResume() {
		...
		Intent intent = getIntent();
		if (intent != null) {
			selectAddressMode = intent
					.hasExtra(SELECT_ADDRESS_POINT_INTENT_KEY);
		} else {
		...
		RegionAddressRepository reg = ((OsmandApplication) getApplication())
				.getResourceManager().getRegionRepository(region);
		if (reg != null
				&& reg.useEnglishNames() != osmandSettings.USE_ENGLISH_NAMES
						.get()) {
