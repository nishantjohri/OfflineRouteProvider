...
public class SearchAddressOnlineActivity extends OsmandListActivity implements
		SearchActivityChild {

	private LatLon location;
	private ProgressDialog progressDlg;
	private final static Log log = OsmLogUtil
			.getLog(SearchAddressOnlineActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		searchButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				inputMethodManager.hideSoftInputFromWindow(searchText
						.getWindowToken(), 0); // Remove keyboard
	...
	protected void searchPlaces(final String search) {
		...
		progressDlg = ProgressDialog.show(this, getString(R.string.searching),
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					b
							.append(
									"?format=xml&addressdetails=0&accept-language=").append(Locale.getDefault().getLanguage()); //$NON-NLS-1$
					...
					conn
							.setRequestProperty(
									"User-Agent", Version.getFullVersion(SearchAddressOnlineActivity.this)); //$NON-NLS-1$
					conn.connect();
					...
					while ((ev = parser.next()) != XmlPullParser.END_DOCUMENT) {
						if (ev == XmlPullParser.START_TAG) {
							if (parser.getName().equals("place")) { //$NON-NLS-1$
								String lat = parser
										.getAttributeValue("", "lat"); //$NON-NLS-1$ //$NON-NLS-2$
								String lon = parser
										.getAttributeValue("", "lon"); //$NON-NLS-1$//$NON-NLS-2$
								String displayName = parser.getAttributeValue(
										"", "display_name"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void showResult(final int warning, final List<Place> places) {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				if (places == null) {
					Toast.makeText(SearchAddressOnlineActivity.this,
							getString(warning), Toast.LENGTH_LONG).show();
				} else {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		settings.setMapLocationToShow(item.lat, item.lon, Math.max(15, settings
				.getLastKnownMapZoom()), getString(R.string.address)
				+ " : " + item.displayName); //$NON-NLS-1$
	...
	class PlacesAdapter extends ArrayAdapter<Place> {
		...
		public PlacesAdapter(List<Place> places) {
			super(SearchAddressOnlineActivity.this,
					R.layout.search_address_online_list_item, places);
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (row == null) {
				...
				row = inflater
						.inflate(R.layout.search_address_online_list_item,
								parent, false);
			}
			...
			if (location != null) {
				int dist = (int) (MapUtils.getDistance(location, model.lat,
						model.lon));
				distanceLabel.setText(OsmAndFormatter.getFormattedDistance(
						dist, SearchAddressOnlineActivity.this));
			} else {
