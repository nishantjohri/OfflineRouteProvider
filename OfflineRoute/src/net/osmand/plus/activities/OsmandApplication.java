...
public class OsmandApplication extends Application {
	public static final String EXCEPTION_PATH = ResourceManager.APP_DIR
			+ "exception.log"; //$NON-NLS-1$
	...
	public void onCreate() {
		...
		// Setting default search value for nFlick Singapore
		osmandSettings.setLastSearchedRegion("Singapore", new LatLon(
				1.322771182802794, 103.84980082511902));
		osmandSettings.setLastSearchedCity(531668011l, "Singapore", new LatLon(
				1.2969947675935738, 103.85530471801758));

		routingHelper = new RoutingHelper(osmandSettings,
				OsmandApplication.this, player);
	...
	public void showDialogInitializingCommandPlayer(final Activity uiContext,
			boolean warningNoneProvider, Runnable run) {
		...
		if (voiceProvider == null
			...
			if (player == null
					|| !Algoritms.objectEquals(voiceProvider, player
							.getCurrentVoice())) {
	...
	private void initVoiceDataInDifferentThread(final Activity uiContext,
			final String voiceProvider, final Runnable run) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					player = CommandPlayerFactory.createCommandPlayer(
	...
	public synchronized void startApplication() {
		...
		startDialog.setRunnable("Initializing app", new Runnable() { //$NON-NLS-1$
					@Override
					public void run() {
						copyOfflineMapData(getApplicationContext());
	...
	private static void copyOfflineMapData(Context context) {
		try {
		...
		try {
			if (osmandSettings.NATIVE_RENDERING.get()) {
				startDialog.startTask(getString(R.string.init_native_library),
						-1);
				RenderingRulesStorage storage = rendererRegistry
			...
			if (helper.hasDataToSave()) {
				startDialog
						.startTask(getString(R.string.saving_gpx_tracks), -1);
			...
			if (bak.exists()
					&& (!save.exists() || bak.lastModified() > save
							.lastModified())) {
				if (save.exists()) {
					save.delete();
				}
					...
					uiHandler.post(new Runnable() {
						...
						public void run() {
							if (toDismiss.getOwnerActivity() != null) {
								toDismiss.getOwnerActivity().dismissDialog(
										PROGRESS_DIALOG);
							}
						}
					});
	...
	private void showWarning(final Context uiContext, final String b) {
		uiHandler.post(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(uiContext, b.toString(), Toast.LENGTH_LONG)
						.show();
			}
		});
	}
			...
			try {
				...
				msg
						...
						append(
								DateFormat
										.format(
												"MMMM dd, yyyy h:mm:ss", System.currentTimeMillis())).append("\n"). //$NON-NLS-1$//$NON-NLS-2$
