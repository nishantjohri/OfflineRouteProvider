...
public class OpeningHoursView {
	...
	public View createOpeningHoursEditView(List<BasicDayOpeningHourRule> t) {
		...
		OnTimeChangedListener onTimeChangedListener = new TimePicker.OnTimeChangedListener() {
			...
			public void onTimeChanged(TimePicker view, int hourOfDay, int minute) {
				if (selectedRule == -1 || !notifyingTime
						|| time.getItem(selectedRule) == null) {
					return;
				}
				if (view == timePickerStart) {
					time.getItem(selectedRule).setStartTime(
							hourOfDay * 60 + minute);
				} else {
					time.getItem(selectedRule).setEndTime(
							hourOfDay * 60 + minute);
				}
	...
	private class TimeAdapter extends ArrayAdapter<BasicDayOpeningHourRule> {
		...
		public View getView(final int position, View convertView,
			...
			if (item == null) {
				...
				text.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						r.setStartTime(timePickerStart.getCurrentHour() * 60
								+ timePickerStart.getCurrentMinute());
						r.setEndTime(timePickerEnd.getCurrentHour() * 60
			...
			if (row == null || row instanceof TextView) {
				LayoutInflater inflater = (LayoutInflater) ctx
						.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
				row = inflater.inflate(R.layout.open_hours_list_item, parent,
						false);
			}
	...
	public void showDaysDialog(final BasicDayOpeningHourRule item,
		...
		b.setMultiChoiceItems(daysToShow, dayToShow,
		...
		b.setPositiveButton(add ? ctx.getString(R.string.default_buttons_add)
		...
		b.setNegativeButton(ctx.getString(R.string.default_buttons_cancel),
				null);
