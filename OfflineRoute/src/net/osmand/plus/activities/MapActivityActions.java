...
public class MapActivityActions implements DialogProvider {
	...
	protected void prepareAddFavouriteDialog(Dialog dialog, Bundle args) {
		...
		final FavouritePoint point = new FavouritePoint(latitude, longitude,
				name, resources.getString(R.string.favorite_default_category));
	...
	protected Dialog createAddFavouriteDialog(final Bundle args) {
		...
		final View v = mapActivity.getLayoutInflater().inflate(
		...
		final AutoCompleteTextView cat = (AutoCompleteTextView) v
				.findViewById(R.id.Category);
		cat.setAdapter(new ArrayAdapter<String>(mapActivity,
		...
		builder.setPositiveButton(R.string.default_buttons_add,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						final FavouritesDbHelper helper = ((OsmandApplication) mapActivity
						...
						if (added) {
							Toast
									.makeText(
											...
											MessageFormat
													.format(
															getString(R.string.add_favorite_dialog_favourite_added_template),
															point.getName()),
	...
	protected Dialog createReplaceFavouriteDialog(final Bundle args) {
		...
		if (names.length == 0) {
			Toast.makeText(mapActivity,
					getString(R.string.fav_points_not_exist),
					Toast.LENGTH_SHORT).show();
			helper.close();
			return null;
		}
		...
		b.setItems(names, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				FavouritePoint point = (FavouritePoint) args
						.getSerializable(KEY_FAVORITE);
				if (helper.editFavourite(fv, point.getLatitude(), point
						.getLongitude())) {
					Toast.makeText(mapActivity,
							getString(R.string.fav_points_edited),
							Toast.LENGTH_SHORT).show();
				}
				mapActivity.getMapView().refreshMap();
			}
		...
		builder.setPositiveButton(R.string.default_buttons_add,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						mapActivity.getSavingTrackHelper().insertPointData(
								latitude, longitude,
								System.currentTimeMillis(), name);
						if (OsmandSettings.getOsmandSettings(mapActivity).SHOW_CURRENT_GPX_TRACK
								.get()) {
							getMyApplication().favorites
									.addFavoritePointToGPXFile(new FavouritePoint(
						...
						Toast
								.makeText(
										...
										MessageFormat
												.format(
														getString(R.string.add_waypoint_dialog_added),
														name),
										Toast.LENGTH_SHORT).show();
						dialog.dismiss();
					}
	...
	protected void reloadTile(final int zoom, final double latitude,
			final double longitude) {
	...
	private Dialog createReloadTitleDialog(final Bundle args) {
		...
		builder.setPositiveButton(R.string.context_menu_item_update_map,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						Rect pixRect = new Rect(0, 0, mapView.getWidth(),
								mapView.getHeight());
						RectF tilesRect = new RectF();
						mapView.calculateTileRectangle(pixRect, mapView
								.getCenterPointX(), mapView.getCenterPointY(),
								mapView.getXTile(), mapView.getYTile(),
								tilesRect);
		...
		builder.setNeutralButton(R.string.context_menu_item_update_poi,
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						updatePoiDb(args.getInt(KEY_ZOOM), args
								.getDouble(KEY_LATITUDE), args
	...
	protected void updatePoiDb(int zoom, double latitude, double longitude) {
		if (zoom < 15) {
			Toast.makeText(mapActivity,
					getString(R.string.update_poi_is_not_available_for_zoom),
					Toast.LENGTH_SHORT).show();
			return;
		}
		final AmenityIndexRepositoryOdb repo = ((OsmandApplication) mapActivity
				.getApplication()).getResourceManager().getUpdatablePoiDb();
		if (repo == null) {
			Toast.makeText(mapActivity,
					getString(R.string.update_poi_no_offline_poi_index),
					Toast.LENGTH_LONG).show();
			return;
		} else {
			Toast.makeText(mapActivity,
					getString(R.string.update_poi_does_not_change_indexes),
					Toast.LENGTH_LONG).show();
		}
		final OsmandMapTileView mapView = mapActivity.getMapView();
		Rect pixRect = new Rect(-mapView.getWidth() / 2,
				-mapView.getHeight() / 2, 3 * mapView.getWidth() / 2,
				3 * mapView.getHeight() / 2);
		RectF tileRect = new RectF();
		mapView.calculateTileRectangle(pixRect, mapView.getCenterPointX(),
				mapView.getCenterPointY(), mapView.getXTile(), mapView
						.getYTile(), tileRect);
		final double leftLon = MapUtils.getLongitudeFromTile(zoom,
				tileRect.left);
		final double topLat = MapUtils.getLatitudeFromTile(zoom, tileRect.top);
		final double rightLon = MapUtils.getLongitudeFromTile(zoom,
				tileRect.right);
		final double bottomLat = MapUtils.getLatitudeFromTile(zoom,
				tileRect.bottom);

		ProgressDialog progressDlg = ProgressDialog.show(mapActivity,
				getString(R.string.loading), getString(R.string.loading_data));
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					boolean loadingPOIs = AmenityIndexRepositoryOdb
							.loadingPOIs(amenities, leftLon, topLat, rightLon,
									bottomLat);
					if (!loadingPOIs) {
						showToast(getString(R.string.update_poi_error_loading));
					} else {
						repo.updateAmenities(amenities, leftLon, topLat,
								rightLon, bottomLat);
						showToast(MessageFormat.format(
								getString(R.string.update_poi_success),
								amenities.size()));
						mapView.refreshMap();
					}
				} catch (Exception e) {
					Log.e(OsmLogUtil.TAG, "Error updating local data", e); //$NON-NLS-1$
	...
	protected void shareLocation(final double latitude, final double longitude,
			int zoom) {
		enhance(dialogBundle, latitude, longitude, zoom);
	...
	private Dialog createShareLocationDialog(final Bundle args) {
		...
		builder.setItems(new String[] { "Email", "SMS", "Clipboard" },
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						final double latitude = args.getDouble(KEY_LATITUDE);
						...
						final String shortOsmUrl = MapUtils.buildShortOsmUrl(
								latitude, longitude, zoom);
						// final String simpleGeo = "geo:"+((float)
						// latitude)+","+((float)longitude) +"?z="+zoom;
						final String appLink = "http://download.osmand.net/go?lat="
								+ ((float) latitude)
								+ "&lon="
								+ ((float) longitude) + "&z=" + zoom;
						if (which == 0) {
							String email = mapActivity.getString(
									R.string.send_location_email_pattern,
									shortOsmUrl, appLink);
							...
							mapActivity.startActivity(Intent.createChooser(
									intent, getString(R.string.send_location)));
						} else {
							String sms = mapActivity.getString(
									R.string.send_location_sms_pattern,
									shortOsmUrl, appLink);
							if (which == 1) {
								...
								ClipboardManager clipboard = (ClipboardManager) mapActivity
	...
	private void prepareAboutRouteDialog(Dialog dlg, Bundle args) {
		((AlertDialog) dlg).setMessage(mapActivity.getRoutingHelper()
	...
	private Dialog createAboutRouteDialog(Bundle args) {
		...
		builder.setMessage(mapActivity.getRoutingHelper()
				.getGeneralRouteInformation());
		builder
				.setPositiveButton(R.string.default_buttons_save,
						saveDirections);
	...
	private boolean checkPointToNavigate() {
		...
		if (mapLayers.getNavigationLayer().getPointToNavigate() == null) {
			Toast.makeText(mapActivity, R.string.mark_final_location_first,
					Toast.LENGTH_LONG).show();
			return false;
		}
		return true;
	...
	protected void getDirections(final double lat, final double lon,
		...
		final OsmandSettings settings = OsmandSettings
				.getOsmandSettings(mapActivity);
		final RoutingHelper routingHelper = mapActivity.getRoutingHelper();

		Builder builder = new AlertDialog.Builder(mapActivity);

		View view = mapActivity.getLayoutInflater().inflate(
				R.layout.calculate_route, null);
		final ToggleButton[] buttons = new ToggleButton[ApplicationMode
				.values().length];
		buttons[ApplicationMode.CAR.ordinal()] = (ToggleButton) view
				.findViewById(R.id.CarButton);
		buttons[ApplicationMode.BICYCLE.ordinal()] = (ToggleButton) view
				.findViewById(R.id.BicycleButton);
		buttons[ApplicationMode.PEDESTRIAN.ordinal()] = (ToggleButton) view
				.findViewById(R.id.PedestrianButton);
		...
		DialogInterface.OnClickListener onlyShowCall = new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				routingHelper.setFinalAndCurrentLocation(mapActivity
						.getPointToNavigate(), location);
			}
		};
		...
		mapLayers.selectGPXFileLayer(new CallbackWithObject<GPXFile>() {
			...
			public boolean processResult(final GPXFile result) {
				...
				builder.setMultiChoiceItems(new String[] {
						...
						props, new OnMultiChoiceClickListener() {
							@Override
				...
				builder.setPositiveButton(R.string.default_buttons_apply,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								GPXRouteParams gpxRoute = new GPXRouteParams(
										result, reverse);

								Location loc = mapActivity
										.getLastKnownLocation();
								if (passWholeWay && loc != null) {
									gpxRoute.setStartPoint(loc);
								}

								Location startForRouting = mapActivity
										.getLastKnownLocation();
								if (startForRouting == null) {
									startForRouting = gpxRoute
								...
								if (endPoint == null || !useDestination) {
									...
									if (endPoint != null) {
										settings.setPointToNavigate(point
												.getLatitude(), point
												.getLongitude(), null);
								...
								if (endPoint != null) {
									settings.FOLLOW_THE_ROUTE.set(true);
									settings.FOLLOW_THE_GPX_ROUTE
											.set(result.path);
									routingHelper.setFollowingMode(true);
									routingHelper
											.setFinalAndCurrentLocation(
													endPoint, startForRouting,
													gpxRoute);
									getMyApplication()
				...
				builder
						.setNegativeButton(R.string.default_buttons_cancel,
								null);
				builder.show();
				return true;
			}
	...
	private ApplicationMode getAppMode(ToggleButton[] buttons,
		...
		for (int i = 0; i < buttons.length; i++) {
			if (buttons[i] != null && buttons[i].isChecked()
					&& i < ApplicationMode.values().length) {
	...
	private Dialog createSaveDirections() {
		...
		final File fileDir = settings
				.extendOsmandPath(ResourceManager.GPX_PATH);
	...
	private class SaveDirectionsAsyncTask extends AsyncTask<File, Void, String> {
		...
		protected String doInBackground(File... params) {
			if (params.length > 0) {
				File file = params[0];
				GPXFile gpx = mapActivity.getRoutingHelper()
						.generateGPXFileWithRoute();
				GPXUtilities.writeGpxFile(file, gpx, mapActivity);
				return mapActivity.getString(
						R.string.route_successfully_saved_at, file.getName());
			}
			return null;
		}
	...
	protected void getDirections(final Location startPoint, final LatLon endPoint) {
		final OsmandSettings settings = OsmandSettings
				.getOsmandSettings(mapActivity);
		final RoutingHelper routingHelper = mapActivity.getRoutingHelper();
		...
		View view = mapActivity.getLayoutInflater().inflate(
				R.layout.calculate_route, null);
		view.findViewById(R.id.BicycleButton).setVisibility(View.GONE);
		final ToggleButton[] buttons = new ToggleButton[2];
		buttons[0] = (ToggleButton) view.findViewById(R.id.CarButton);
		buttons[1] = (ToggleButton) view.findViewById(R.id.PedestrianButton);
		...
		DialogInterface.OnClickListener onlyShowCall = new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				mapActivity.navigateToPoint(startPoint, endPoint);
			}
		};

		builder.setView(view);
		builder.setTitle(R.string.get_directions);
		builder.setPositiveButton(R.string.get_directions, onlyShowCall);
		builder.setNegativeButton(R.string.default_buttons_cancel, null);
		builder.show();
	}
}
