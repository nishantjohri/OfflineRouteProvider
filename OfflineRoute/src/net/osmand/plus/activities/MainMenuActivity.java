...
public class MainMenuActivity extends TrackedActivity {
	...
	private static final String EXCEPTION_FILE_SIZE = ResourceManager.APP_DIR
	...
	public void checkPreviousRunsForExceptions(boolean firstTime) {
		...
		final File file = OsmandSettings.getOsmandSettings(this)
				.extendOsmandPath(OsmandApplication.EXCEPTION_PATH);
		if (file.exists() && file.length() > 0) {
			...
			getPreferences(MODE_WORLD_WRITEABLE).edit().putLong(
					EXCEPTION_FILE_SIZE, file.length()).commit();
		} else {
			if (size > 0) {
				getPreferences(MODE_WORLD_WRITEABLE).edit().putLong(
						EXCEPTION_FILE_SIZE, 0).commit();
			}
		}
	}
		...
		if (prefs.contains(CONTRIBUTION_VERSION_FLAG)) {
			...
			content.setSpan(new ClickableSpan() {
				...
				public void onClick(View widget) {
					final Intent mapIntent = new Intent(activity,
							ContributionVersionActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!pref.contains(FIRST_TIME_APP_RUN)) {
			...
			pref.edit().putString(VERSION_INSTALLED,
