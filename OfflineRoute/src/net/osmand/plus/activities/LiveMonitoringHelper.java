...
public class LiveMonitoringHelper {
	...
	private final static Log log = OsmLogUtil
			.getLog(LiveMonitoringHelper.class);
	...
	public void insertData(double lat, double lon, double alt, double speed,
		...
		if (time - lastTimeUpdated > settings.LIVE_MONITORING_INTERVAL.get() * 1000) {
			LiveMonitoringData data = new LiveMonitoringData((float) lat,
	...
	private static class LiveMonitoringData {
		...
		public LiveMonitoringData(float lat, float lon, float alt, float speed,
				float hdop, long time) {
			this.lat = lat;
			this.lon = lon;
			this.alt = alt;
			this.speed = speed;
	...
	public void sendData(LiveMonitoringData data) {
		String url = MessageFormat.format(settings.LIVE_MONITORING_URL.get(),
				data.lat + "", data.lon + "", data.time + "", data.hdop + "",
				data.alt + "", data.speed + "");
		try {
