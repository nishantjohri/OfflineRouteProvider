...
public class NavigatePointActivity extends TrackedActivity implements
		SearchActivityChild {
	Dialog dlg;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		((Button) findViewById(R.id.Cancel))
				.setText(getString(R.string.navigate_to));
	}
	...
	protected void onResume() {
		...
		if (loc == null) {
			loc = OsmandSettings.getOsmandSettings(this)
	...
	public void initUI(double latitude, double longitude) {
		...
		((RadioGroup) findViewById(R.id.RadioGroup))
				.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
					...
					public void onCheckedChanged(RadioGroup group, int checkedId) {
						...
						try {
							double lat = convert(((TextView) findViewById(R.id.LatitudeEdit))
									.getText().toString());
							double lon = convert(((TextView) findViewById(R.id.LongitudeEdit))
									.getText().toString());
							((TextView) findViewById(R.id.ValidateTextView))
							...
							Log.w(OsmLogUtil.TAG, "Convertion failed", e); //$NON-NLS-1$
						}
					}
		...
		((Button) findViewById(R.id.ShowOnMap))
				.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						showOnMap(false);
		...
		TextWatcher textWatcher = new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				pasteString = null;
				if (count > 3) {
					pasteString = s.subSequence(start, start + count)
							.toString();
				}
			}
			...
			public void afterTextChanged(Editable s) {
				...
				if (pasteString != null) {
					...
					if (lonSt != -1) {
						...
						try {
							String latString = pasteString.substring(latSt,
									latEnd);
							String lonString = pasteString.substring(lonSt,
									lonEnd);
	...
	public void showOnMap(boolean navigate) {
		try {
			...
			if (navigate) {
				OsmandSettings.getOsmandSettings(this).setPointToNavigate(lat,
						lon, getString(R.string.point_on_map, lat, lon));
			} else {
				// in case when it is dialog
				if (activity != null) {
					OsmandMapTileView v = activity.getMapView();
					v.getAnimatedDraggingThread().startMoving(lat, lon,
							v.getZoom(), true);
				} else {
					OsmandSettings settings = OsmandSettings
							.getOsmandSettings(this);
					settings.setMapLocationToShow(lat, lon, Math.max(12,
							settings.getLastKnownMapZoom()), getString(
							R.string.point_on_map, lat, lon));
				}
			}
			close();
		} catch (RuntimeException e) {
	...
	// and to fix bug in android implementation : doesn't convert if min = 59.23
	// or sec = 59.32 or deg=179.3
	...
	public static String convert(double coordinate, int outputType) {
		...
		if ((outputType != FORMAT_DEGREES) && (outputType != FORMAT_MINUTES)
