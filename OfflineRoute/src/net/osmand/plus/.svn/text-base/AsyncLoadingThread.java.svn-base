...
public class AsyncLoadingThread extends Thread {
	...
	private static final Log log = OsmLogUtil.getLog(AsyncLoadingThread.class);
	...
	private int calculateProgressStatus() {
		int progress = 0;
		if (resourceManger.getMapTileDownloader() != null
				&& resourceManger.getMapTileDownloader()
						.isSomethingBeingDownloaded()) {
			progress = BusyIndicator.STATUS_GREEN;
		} else if (resourceManger.getContext().getRoutingHelper()
				.isRouteBeingCalculated()) {
			...
			progress = BusyIndicator.STATUS_BLACK;
		} else if (transportLoadRequest != null
				&& transportLoadRequest.isRunning()) {
			progress = BusyIndicator.STATUS_BLACK;
		}
		return progress;
	}
			...
			try {
				...
				synchronized (resourceManger) {
					if (resourceManger.getBusyIndicator() != null) {
						resourceManger.getBusyIndicator()
				...
				while (!requests.isEmpty()) {
					...
					if (req instanceof TileLoadDownloadRequest) {
						...
						if (!amenityLoaded) {
							if (poiLoadRequest == null
									|| asyncLoadingPoi == null) {
								startPoiLoadingThread();
								poiLoadRequest = (AmenityLoadRequest) req;
								asyncLoadingPoi.post(poiLoadRequest
										.prepareToRun());
							} else if (poiLoadRequest
									.recalculateRequest((AmenityLoadRequest) req)) {
								poiLoadRequest = (AmenityLoadRequest) req;
								asyncLoadingPoi.post(poiLoadRequest
										.prepareToRun());
							}
						...
						if (!transportLoaded) {
							if (transportLoadRequest == null
									|| asyncLoadingTransport == null) {
								startTransportLoadingThread();
								transportLoadRequest = (TransportLoadRequest) req;
								asyncLoadingTransport.post(transportLoadRequest
										.prepareToRun());
							} else if (transportLoadRequest
									.recalculateRequest((TransportLoadRequest) req)) {
								transportLoadRequest = (TransportLoadRequest) req;
								asyncLoadingTransport.post(transportLoadRequest
										.prepareToRun());
							}
						...
						if (!mapLoaded) {
							...
							resourceManger.getRenderer().loadMap(
									r.tileBox,
									resourceManger.getMapTileDownloader()
				...
				if (tileLoaded || amenityLoaded || transportLoaded || mapLoaded) {
					...
					for (IMapDownloaderCallback c : resourceManger
							.getMapTileDownloader().getDownloaderCallbacks()) {
				...
				if (progress != newProgress) {
					synchronized (resourceManger) {
						if (resourceManger.getBusyIndicator() != null) {
							resourceManger.getBusyIndicator().updateStatus(
									newProgress);
						}
					}
				}
	...
	public boolean isFileCurrentlyDownloaded(File fileToSave) {
		return resourceManger.getMapTileDownloader().isFileCurrentlyDownloaded(
				fileToSave);
	}
	...
	protected static class TileLoadDownloadRequest extends DownloadRequest {
		...
		public TileLoadDownloadRequest(File dirWithTiles, String url,
				File fileToSave, String tileId, ITileSource source, int tileX,
				int tileY, int zoom) {
	...
	protected class MapObjectLoadRequest<T> implements ResultMatcher<T> {
		...
		public boolean isContains(double topLatitude, double leftLongitude,
				double bottomLatitude, double rightLongitude) {
			boolean inside = this.topLatitude >= topLatitude
		...
		public void setBoundaries(double topLatitude, double leftLongitude,
				double bottomLatitude, double rightLongitude) {
	...
	protected class AmenityLoadRequest extends MapObjectLoadRequest<Amenity> {
		...
		public AmenityLoadRequest(List<AmenityIndexRepository> repos, int zoom,
				PoiFilter filter) {
			super();
			this.res = repos;
			this.zoom = zoom;
		...
		public Runnable prepareToRun() {
			...
			final double nleftLongitude = leftLongitude
					- (rightLongitude - leftLongitude) / 2;
			final double nrightLongitude = rightLongitude
					+ (rightLongitude - leftLongitude) / 2;
			setBoundaries(ntopLatitude, nleftLongitude, nbottomLatitude,
					nrightLongitude);
			return new Runnable() {
				@Override
				public void run() {
					...
					try {
						for (AmenityIndexRepository repository : res) {
							repository.evaluateCachedAmenities(ntopLatitude,
									nleftLongitude, nbottomLatitude,
									nrightLongitude, zoom, filter,
		...
		public boolean recalculateRequest(AmenityLoadRequest req) {
			if (this.zoom != req.zoom
					|| !Algoritms.objectEquals(this.filter, req.filter)) {
				return true;
			}
			return !isContains(req.topLatitude, req.leftLongitude,
					req.bottomLatitude, req.rightLongitude);
		}

	}

	protected class TransportLoadRequest extends
			MapObjectLoadRequest<TransportStop> {
		private final List<TransportIndexRepository> repos;
		private int zoom;

		public TransportLoadRequest(List<TransportIndexRepository> repos,
				int zoom) {
			super();
			this.repos = repos;
			this.zoom = zoom;
				...
				public void run() {
					...
					try {
						for (TransportIndexRepository repository : repos) {
							repository.evaluateCachedTransportStops(
									ntopLatitude, nleftLongitude,
									nbottomLatitude, nrightLongitude, zoom,
