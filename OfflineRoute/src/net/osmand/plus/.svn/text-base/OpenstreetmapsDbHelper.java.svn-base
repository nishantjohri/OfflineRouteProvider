...
public class OpenstreetmapsDbHelper extends SQLiteOpenHelper {
	...
	private static final String OPENSTREETMAP_TABLE_CREATE = "CREATE TABLE " + OPENSTREETMAP_TABLE_NAME + " (" + //$NON-NLS-1$ //$NON-NLS-2$
			...
			+ " INTEGER, " + OPENSTREETMAP_COL_NAME + " TEXT, " + OPENSTREETMAP_COL_TYPE + " TEXT, " + OPENSTREETMAP_COL_SUBTYPE + " TEXT, " + //$NON-NLS-1$ //$NON-NLS-2$ 
			OPENSTREETMAP_COL_LAT
			+ " double, " + OPENSTREETMAP_COL_LON + " double, " + //$NON-NLS-1$ //$NON-NLS-2$
			OPENSTREETMAP_COL_ACTION
			+ " TEXT, " + OPENSTREETMAP_COL_COMMENT + " TEXT, " + OPENSTREETMAP_COL_OPENINGHOURS + " TEXT);"; //$NON-NLS-1$ //$NON-NLS-2$ 
	...
	public boolean addOpenstreetmap(OpenstreetmapPoint p) {
		...
		if (db != null) {
			db
					.execSQL(
							"INSERT INTO " + OPENSTREETMAP_TABLE_NAME + " ("
									+ OPENSTREETMAP_COL_ID + ", "
									+ OPENSTREETMAP_COL_NAME + ", "
									+ OPENSTREETMAP_COL_TYPE + ", "
									+ OPENSTREETMAP_COL_SUBTYPE + ", "
									+ OPENSTREETMAP_COL_LAT + ","
	...
	public boolean deleteOpenstreetmap(OpenstreetmapPoint p) {
		...
		SQLiteDatabase db = getWritableDatabase();
		if (db != null) {
			db.execSQL("DELETE FROM " + OPENSTREETMAP_TABLE_NAME + " WHERE "
					+ OPENSTREETMAP_COL_ID + " = ? AND "
					...
					+ OPENSTREETMAP_COL_OPENINGHOURS + " = ?", new Object[] {
					p.getId(), p.getName(), p.getType(), p.getSubtype(),
					p.getLatitude(), p.getLongitude(),
					OpenstreetmapRemoteUtil.stringAction.get(p.getAction()),
					p.getComment(), p.getOpeninghours() }); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void checkOpenstreetmapPoints() {
		...
		if (db != null) {
			Cursor query = db
					.rawQuery(
							"SELECT " + OPENSTREETMAP_COL_ID + ", " + OPENSTREETMAP_COL_NAME + ", " + OPENSTREETMAP_COL_TYPE + ", " + OPENSTREETMAP_COL_SUBTYPE + ", " + OPENSTREETMAP_COL_LAT + "," + OPENSTREETMAP_COL_LON + "," + OPENSTREETMAP_COL_ACTION + "," + OPENSTREETMAP_COL_COMMENT + "," + OPENSTREETMAP_COL_OPENINGHOURS + " FROM " + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ 
			...
			if (query.moveToFirst()) {
				do {
					...
					Node entity = new Node(query.getDouble(4), query
							.getDouble(5), query.getLong(0));
					...
					if (openingHours != null && openingHours.length() > 0)
						entity.putTag(OSMTagKey.OPENING_HOURS.getValue(),
								openingHours);
					p.setEntity(entity);
					p.setStored(true);
