...
public class AmenityIndexRepositoryOdb extends
		BaseLocationIndexRepository<Amenity> implements AmenityIndexRepository {
	private static final Log log = OsmLogUtil
			.getLog(AmenityIndexRepositoryOdb.class);
	...
	private final String[] columns = new String[] {
			"id", "x", "y", "name", "name_en", "type", "subtype", "opening_hours", "phone", "site" }; //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$//$NON-NLS-4$//$NON-NLS-5$//$NON-NLS-6$//$NON-NLS-7$//$NON-NLS-8$ //$NON-NLS-9$ //$NON-NLS-10$

	@Override
	public List<Amenity> searchAmenities(int stop, int sleft, int sbottom,
			int sright, int zoom, PoiFilter filter, List<Amenity> amenities,
			...
			log
					.debug(String
							.format(
									...
									MapUtils.get31LatitudeY(stop)
											+ " " + MapUtils.get31LongitudeX(sleft), System.currentTimeMillis() - now, amenities.size())); //$NON-NLS-1$
		}
		return amenities;
	}
	...
	public void evaluateCachedAmenities(double topLatitude,
			...
			int zoom, PoiFilter filter, ResultMatcher<Amenity> matcher) {
	...
	public synchronized boolean checkCachedAmenities(double topLatitude,
		...
		boolean noNeedToSearch = inside
				&& Algoritms.objectEquals(filterId, cFilterId);
	...
	public boolean checkCachedAmenities(double topLatitude,
			double leftLongitude, double bottomLatitude, double rightLongitude,
			int zoom, String filterId, List<Amenity> toFill) {
		return checkCachedAmenities(topLatitude, leftLongitude, bottomLatitude,
				rightLongitude, zoom, filterId, toFill, false);
	}

	public boolean initialize(final IProgress progress, File file) {
		return super.initialize(progress, file,
				IndexConstants.POI_TABLE_VERSION, IndexConstants.POI_TABLE,
				true);
	}
	...
	public boolean updateAmenity(Amenity a) {
		...
		db.execSQL(b.toString(), new Object[] {
				MapUtils.get31TileNumberX(a.getLocation().getLongitude()),
				MapUtils.get31TileNumberY(a.getLocation().getLatitude()),
				a.getOpeningHours(), a.getName(), a.getEnName(),
				AmenityType.valueToString(a.getType()), a.getSubType(),
	...
	public boolean deleteAmenities(long id) {
		db
				.execSQL("DELETE FROM " + IndexConstants.POI_TABLE + " WHERE id=" + id); //$NON-NLS-1$ //$NON-NLS-2$
		return true;
	}

	public boolean updateAmenities(List<Amenity> amenities, double leftLon,
			double topLat, double rightLon, double bottomLat) {
	...
	private void insertAmenities(Collection<Amenity> amenities) {
		SQLiteStatement stat = db
				.compileStatement("DELETE FROM " + IndexConstants.POI_TABLE + " WHERE id = ?"); //$NON-NLS-1$//$NON-NLS-2$
		...
		stat.close();
		stat = db
				.compileStatement("INSERT INTO " + IndexConstants.POI_TABLE + //$NON-NLS-1$
						"(id, x, y, name_en, name, type, subtype, opening_hours, site, phone) values(?,?,?,?,?,?,?,?,?,?)"); //$NON-NLS-1$
		for (Amenity a : amenities) {
			stat.bindLong(1, a.getId());
			stat.bindDouble(2, MapUtils.get31TileNumberX(a.getLocation()
					.getLongitude()));
			stat.bindDouble(3, MapUtils.get31TileNumberY(a.getLocation()
					.getLatitude()));
			dataBottomLatitude = Math.min(a.getLocation().getLatitude() - 0.5,
					dataBottomLatitude);
			dataTopLatitude = Math.max(a.getLocation().getLatitude() + 0.5,
					dataTopLatitude);
			dataLeftLongitude = Math.min(a.getLocation().getLongitude() - 0.5,
					dataLeftLongitude);
			dataRightLongitude = Math.max(a.getLocation().getLongitude() + 0.5,
					dataRightLongitude);
	...
	public static void createAmenityIndexRepository(File file) {
		...
		db
				.execSQL("create table " + IndexConstants.POI_TABLE + //$NON-NLS-1$
	...
	public static boolean loadingPOIs(List<Amenity> amenities, double leftLon,
			double topLat, double righLon, double bottomLat) {
		try {
			// bbox=left,bottom,right,top
			String u = SITE_API
					+ "api/0.6/map?bbox=" + leftLon + "," + bottomLat + "," + righLon + "," + topLat; //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$//$NON-NLS-4$
			URL url = new URL(u);
			...
			st.getFilters().add(new IOsmStorageFilter() {
				@Override
				public boolean acceptEntityToLoad(OsmBaseStorage storage,
						Entity.EntityId id, Entity entity) {
