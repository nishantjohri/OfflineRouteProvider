...
public class SQLiteTileSource implements ITileSource {
	...
	private static final Log log = OsmLogUtil.getLog(SQLiteTileSource.class);
	...
	private SQLiteDatabase getDatabase() {
		if (db == null && file.exists()) {
			...
			try {
				String template = db
						.compileStatement("SELECT url FROM info").simpleQueryForString(); //$NON-NLS-1$
			...
			try {
				long z;
				z = db
						.compileStatement("SELECT minzoom FROM info").simpleQueryForLong(); //$NON-NLS-1$
				if (z < 17 && z >= 0)
					maxZoom = 17 - (int) z;
				z = db
						.compileStatement("SELECT maxzoom FROM info").simpleQueryForLong(); //$NON-NLS-1$
				if (z < 17 && z >= 0)
					minZoom = 17 - (int) z;
			} catch (RuntimeException e) {
			}
	...
	public boolean exists(int zoom) {
		...
		Cursor cursor = db
				.rawQuery(
						"SELECT 1 FROM tiles WHERE z = ? LIMIT 1", new String[] { (17 - zoom) + "" }); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$//$NON-NLS-4$
		try {
	...
	public boolean exists(int x, int y, int zoom) {
		...
		Cursor cursor = db
				.rawQuery(
						"SELECT 1 FROM tiles WHERE x = ? AND y = ? AND z = ?", new String[] { x + "", y + "", (17 - zoom) + "" }); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$//$NON-NLS-4$
		try {
			boolean e = cursor.moveToFirst();
			cursor.close();
			if (log.isDebugEnabled()) {
				log
						.debug("Checking tile existance x = " + x + " y = " + y + " z = " + zoom + " for " + (System.currentTimeMillis() - time)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
			}
			return e;
		} catch (SQLiteDiskIOException e) {
			return false;
	...
	public Bitmap getImage(int x, int y, int zoom) {
		...
		Cursor cursor = db
				.rawQuery(
						"SELECT image FROM tiles WHERE x = ? AND y = ? AND z = ?", new String[] { x + "", y + "", (17 - zoom) + "" }); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$//$NON-NLS-4$
		byte[] blob = null;
	...
	public void deleteImage(int x, int y, int zoom) {
		...
		db
				.execSQL(
						"DELETE FROM tiles WHERE x = ? AND y = ? AND z = ?", new String[] { x + "", y + "", (17 - zoom) + "" }); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$//$NON-NLS-4$
	}
	 ...
	 */
	public synchronized void insertImage(int x, int y, int zoom, File fileToSave)
			throws IOException {
		SQLiteDatabase db = getDatabase();
		if (db == null || db.isReadOnly()) {
			return;
		...
		SQLiteStatement statement = db
				.compileStatement("INSERT INTO tiles VALUES(?, ?, ?, ?, ?)"); //$NON-NLS-1$
