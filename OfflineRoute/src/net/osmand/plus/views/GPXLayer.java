...
public class GPXLayer extends OsmandMapLayer {
	...
	public void onDraw(Canvas canvas, RectF latLonBounds, RectF tilesRect,
		...
		for (List<WptPt> l : points) {
			...
			for (int i = 0; i < l.size(); i++) {
				WptPt ls = l.get(i);
				if (startIndex == -1) {
					if (ls.lat >= latLonBounds.bottom
							&& ls.lat <= latLonBounds.top
							&& ls.lon >= latLonBounds.left
							&& ls.lon <= latLonBounds.right) {
						startIndex = i > 0 ? i - 1 : i;
					}
				} else if (!(latLonBounds.left <= ls.lon + 0.03
						&& ls.lon - 0.03 <= latLonBounds.right
						&& latLonBounds.bottom <= ls.lat + 0.03 && ls.lat - 0.03 <= latLonBounds.top)) {
					drawSegment(canvas, l, startIndex, i);
					// do not continue make method more efficient (because it
					// calls in UI thread)
	...
	private void drawSegment(Canvas canvas, List<WptPt> l, int startIndex,
			int endIndex) {
	...
	public void addTrackPoint(WptPt pt) {
		...
		if (last.size() == 0
				|| last.get(last.size() - 1).time - pt.time < 6 * 60 * 1000) {
