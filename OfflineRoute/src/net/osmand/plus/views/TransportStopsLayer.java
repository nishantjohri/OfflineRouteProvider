...
public class TransportStopsLayer extends OsmandMapLayer implements
	...
	private String getStopDescription(TransportStop n, boolean useName) {
		...
		text
				.append(view.getContext().getString(R.string.transport_Stop))
		...
		List<TransportIndexRepository> reps = view.getApplication()
	...
	public void onDraw(Canvas canvas, RectF latLonBounds, RectF tilesRect,
		...
		if (view.getZoom() >= startZoom) {
			objects.clear();
			view.getApplication().getResourceManager().searchTransportAsync(
					latLonBounds.top, latLonBounds.left, latLonBounds.bottom,
					latLonBounds.right, view.getZoom(), objects);
