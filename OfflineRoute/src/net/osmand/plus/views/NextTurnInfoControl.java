...
public class NextTurnInfoControl extends MapInfoControl {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		int h = (int) (5 * scaleCoefficient + Math.max(textPaint.getTextSize(),
				subtextPaint.getTextSize()));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (pathForTurn != null) {
			...
			if (exitOut != null) {
				drawShadowText(canvas, exitOut, (getWWidth()) / 2 - 7
						* scaleCoefficient, getWHeight() / 2
						- textPaint.getTextSize() / 2 + 3 * scaleCoefficient,
						textPaint);
			}
			String text = OsmAndFormatter.getFormattedDistance(
					nextTurnDirection, getContext());
			String subtext = null;

			// Issue 863: distance "as soon as possible" should be displayed for
			// unscheduled U-turn
			...
			float startX = Math.max((getWWidth() - st - mt) / 2,
					2 * scaleCoefficient);
			drawShadowText(canvas, text, startX, getWHeight() - 5
					* scaleCoefficient, textPaint);
			if (subtext != null) {
				drawShadowText(canvas, subtext, startX + 2 * scaleCoefficient
						+ mt, getWHeight() - 5 * scaleCoefficient, subtextPaint);
			}
		}
	}
