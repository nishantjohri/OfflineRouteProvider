...
public class MapInfoLayer extends OsmandMapLayer {
	...
	public void initLayer(final OsmandMapTileView view) {
		this.view = view;
		WindowManager mgr = (WindowManager) view.getContext().getSystemService(
	...
	public void applyTheme() {
		...
		leftStack.setExpandImageDrawable(view.getResources()
				.getDrawable(expand));
		rightStack.setExpandImageDrawable(view.getResources().getDrawable(
				expand));
		statusBar
				.setBackgroundDrawable(view.getResources().getDrawable(boxTop));
	...
	public void createTopBarElements() {
		...
		statusBar.setBackgroundDrawable(view.getResources().getDrawable(
				R.drawable.box_top));
		...
		view.getResources().getDrawable(R.drawable.box_top).getPadding(
				topRectPadding);
	...
	public boolean distChanged(int oldDist, int dist) {
		if (oldDist != 0 && oldDist - dist < 100
				&& Math.abs(((float) dist - oldDist) / oldDist) < 0.01) {
			return false;
		}
		return true;
	}
	...
	private TextInfoControl createSpeedControl() {
		final TextInfoControl speedControl = new TextInfoControl(map, 3,
			...
			public boolean updateInfo() {
				...
				if (map.getLastKnownLocation() != null
						&& map.getLastKnownLocation().hasSpeed()) {
					if (Math.abs(map.getLastKnownLocation().getSpeed()
							- cachedSpeed) > .3f) {
						cachedSpeed = map.getLastKnownLocation().getSpeed();
						String ds = OsmAndFormatter.getFormattedSpeed(
								cachedSpeed, map);
	...
	private TextInfoControl createAltitudeControl() {
		final TextInfoControl altitudeControl = new TextInfoControl(map, 0,
				paintText, paintSubText) {
			...
			public boolean updateInfo() {
				// draw speed
				if (showAltitude && map.getLastKnownLocation() != null
						&& map.getLastKnownLocation().hasAltitude()) {
					if (cachedAlt != (int) map.getLastKnownLocation()
							.getAltitude()) {
						cachedAlt = (int) map.getLastKnownLocation()
								.getAltitude();
		...
		altitudeControl.setImageDrawable(view.getResources().getDrawable(
				R.drawable.ic_altitude));
		return altitudeControl;
	}

	private TextInfoControl createTimeControl() {
		final Drawable time = view.getResources().getDrawable(
				R.drawable.info_time);
		final Drawable timeToGo = view.getResources().getDrawable(
				R.drawable.info_time_to_go);
		showArrivalTime = view.getSettings().SHOW_ARRIVAL_TIME_OTHERWISE_EXPECTED_TIME
				.get();
		final TextInfoControl leftTimeControl = new TextInfoControl(map, 0,
				paintText, paintSubText) {
			...
			public boolean updateInfo() {
				int time = 0;
				if (routeLayer != null
						&& routeLayer.getHelper().isRouterEnabled()) {
					boolean followingMode = routeLayer.getHelper()
							.isFollowingMode();
					time = routeLayer.getHelper().getLeftTime();
					if (time != 0) {
						if (followingMode && showArrivalTime) {
							long toFindTime = time * 1000
							...
							if (Math.abs(toFindTime - cachedLeftTime) > 30000) {
								...
								if (DateFormat.is24HourFormat(map)) {
									setText(
											DateFormat
													.format("k:mm", toFindTime).toString(), null); //$NON-NLS-1$
								} else {
									setText(
											DateFormat.format("h:mm",
													toFindTime).toString(),
											DateFormat
													.format("aa", toFindTime).toString()); //$NON-NLS-1$
								}
								return true;
							}
						} else {
							if (Math.abs(time - cachedLeftTime) > 30) {
								calculateRouteBtn.setText(R.string.lbl_stop_routing_btn);
								cachedLeftTime = time;
								...
								setText(
										String
												.format(
														"%d:%02d:%02d", hours, minutes, seconds), null); //$NON-NLS-1$
								return true;
							}
						}
					}
		...
		leftTimeControl.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				leftTimeControl.setImageDrawable(showArrivalTime ? time
						: timeToGo);
				view.getSettings().SHOW_ARRIVAL_TIME_OTHERWISE_EXPECTED_TIME
						.set(showArrivalTime);
				view.refreshMap();
			}
	...
	private TextInfoControl createDistanceControl() {
		TextInfoControl distanceControl = new TextInfoControl(map, 0,
				paintText, paintSubText) {
			...
			public boolean updateInfo() {
				if (map.getPointToNavigate() != null) {
					...
					if (d == 0) {
						Location.distanceBetween(view.getLatitude(), view
								.getLongitude(), map.getPointToNavigate()
								.getLatitude(), map.getPointToNavigate()
								.getLongitude(), calculations);
					...
					if (distChanged(cachedMeters, d)) {
						...
						if (cachedMeters <= 20) {
							...
							String ds = OsmAndFormatter.getFormattedDistance(
									cachedMeters, map);
							int ls = ds.lastIndexOf(' ');
							if (ls == -1) {
								setText(ds, null);
							} else {
								setText(ds.substring(0, ls), ds
										.substring(ls + 1));
							}
						}
						return true;
		...
		distanceControl.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				LatLon pointToNavigate = view.getSettings()
						.getPointToNavigate();
				if (pointToNavigate != null) {
					int fZoom = view.getZoom() < 15 ? 15 : view.getZoom();
					thread.startMoving(pointToNavigate.getLatitude(),
							pointToNavigate.getLongitude(), fZoom, true);
				}
			}
		});
		distanceControl.setText(null, null);
		distanceControl.setImageDrawable(view.getResources().getDrawable(
				R.drawable.info_target));
	...
	private NextTurnInfoControl createNextInfoControl() {
		final NextTurnInfoControl nextTurnInfo = new NextTurnInfoControl(map,
				paintText, paintSubText) {
			...
			public boolean updateInfo() {
				...
				if (routeLayer != null
						&& routeLayer.getHelper().isRouterEnabled()
						&& routeLayer.getHelper().isFollowingMode()) {
					int d = routeLayer.getHelper()
							.getDistanceToNextRouteDirection();
					...
					if (routeLayer.getHelper().makeUturnWhenPossible() == true) {
						if (!showMiniMap) {
							...
							TurnPathHelper.calcTurnPath(pathForTurn, turnType,
									pathTransform);
							invalidate();
						}
					} else {
						if (d > 0 && !showMiniMap) {
							visible = true;
							RouteDirectionInfo next = routeLayer.getHelper()
									.getNextRouteDirectionInfo();
							if (next == null) {
								if (turnType != null) {
									turnType = null;
									invalidate();
								}
							} else if (!Algoritms.objectEquals(turnType,
									next.turnType)) {
								turnType = next.turnType;
								TurnPathHelper.calcTurnPath(pathForTurn,
										turnType, pathTransform);
		...
		nextTurnInfo.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// i++;
				// if (i % (l + exits) >= l ) {
				// nextTurnInfo.turnType = TurnType.valueOf("EXIT" + (i % (l +
				// exits) - l + 1));
				// float a = 180 - (i % (l + exits) - l + 1) * 50;
				// nextTurnInfo.turnType.setTurnAngle(a < 0 ? a + 360 : a);
				// } else {
				// nextTurnInfo.turnType =
				// TurnType.valueOf(TurnType.predefinedTypes[i %
				// (TurnType.predefinedTypes.length + exits)]);
				// }
	...
	private ViewGroup createStatusBar() {
		...
		// Navigate button
		navigateButton = new ToggleButton(view.getContext());
		navigateButton.setText(R.string.navigate_btn);
		LayoutParams layoutParams = new LinearLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		layoutParams.gravity = Gravity.CENTER_VERTICAL;
		navigateButton.setLayoutParams(layoutParams);
		navigateButton.setTextOn("Navigate");
		navigateButton.setTextOff("Navigate");
		// navigateButton.setBackgroundColor(Color.TRANSPARENT);
		...
		statusBar.addView(navigateButton);
		
		//Calculate route button
		calculateRouteBtn = new Button(view.getContext());
		calculateRouteBtn.setText(R.string.lbl_calculate_route_btn);
		layoutParams = new LinearLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		layoutParams.gravity = Gravity.CENTER_VERTICAL;
		calculateRouteBtn.setLayoutParams(layoutParams);
		calculateRouteBtn.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				Button button = (Button)v;
				String string = v.getContext().getString(R.string.lbl_stop_routing_btn);
				if(button.getText().equals(string)){
					map.getMapLayers().getRouteLayer().setPoints(map.locations, true);
					view.refreshMap();
					map.clearRoute();
					calculateRouteBtn.setText(R.string.lbl_calculate_route_btn);
				}else{
		...
		statusBar.addView(calculateRouteBtn);
		

		// Compass icon
		final Drawable compass = view.getResources().getDrawable(
				R.drawable.compass);
		...
		infoText = new TextView(view.getContext());
		statusBar.addView(infoText, params);
		...
		params = new LinearLayout.LayoutParams(
				globusDrawable.getMinimumWidth(), globusDrawable
						.getMinimumHeight());
		...
		backToLocation.setImageDrawable(view.getResources().getDrawable(
