...
public class POIMapLayer extends OsmandMapLayer implements
		ContextMenuLayer.IContextMenuProvider {
	private static final int startZoom = 10;
	public static final int TEXT_WRAP = 15;
	public static final int TEXT_LINES = 3;
	public static final org.apache.commons.logging.Log log = OsmLogUtil
			.getLog(POIMapLayer.class);
	...
	private ArrayList<MapPoi> pois;
	private boolean isMapPoi = false;
		...
		if (objects != null) {
			...
			try {
				for (int i = 0; i < objects.size(); i++) {
					...
					int x = view.getRotatedMapXForPoint(n.getLocation()
							.getLatitude(), n.getLocation().getLongitude());
					int y = view.getRotatedMapYForPoint(n.getLocation()
							.getLatitude(), n.getLocation().getLongitude());
					if (Math.abs(x - ex) <= compare
							&& Math.abs(y - ey) <= compare) {
						compare = radius;
						am.add(n);
					}
				}
	...
	public boolean onSingleTap(PointF point) {
		if (isMapPoi) {
			List<MapPoi> poiList = new ArrayList<MapPoi>();
			getPoiFromPoint(point, poiList);
			if (!poiList.isEmpty()) {
				...
				MapPoi poi = poiList.get(0);
				LatLon latLon = new LatLon(poi.getLatitude(),
						poi.getLongitude());
				activity.getMapLayers().getContextMenuLayer().setPoi(poi);
				activity.getMapLayers().getContextMenuLayer()
						.setLocation(latLon, "\n" + sb.toString() + "\n");
				view.refreshMap();
				return true;
			}
			...
			if (!am.isEmpty()) {
				...
				Toast.makeText(view.getContext(), res.toString(),
	...
	private StringBuilder buildPoiInformation(StringBuilder res, Amenity n) {
		String format = OsmAndFormatter.getPoiSimpleFormat(n,
				view.getContext(), view.getSettings().USE_ENGLISH_NAMES.get());
		res.append(" " + format);
	...
	public void onDraw(Canvas canvas, RectF latLonBounds, RectF tilesRect,
		...
		if (view.getZoom() >= startZoom) {
			if (isMapPoi) {
				drawItineraryPoi(canvas);
			}
			/*
			 * else{ objects.clear();
			 * resourceManager.searchAmenitiesAsync(latLonBounds.top,
			 * latLonBounds.left, latLonBounds.bottom, latLonBounds.right,
			 * view.getZoom(), filter, objects);
			 * 
			 * int r = getRadiusPoi(view.getZoom()); for (Amenity o : objects) {
			 * int x =
			 * view.getRotatedMapXForPoint(o.getLocation().getLatitude(),
			 * o.getLocation().getLongitude()); int y =
			 * view.getRotatedMapYForPoint(o.getLocation().getLatitude(),
			 * o.getLocation().getLongitude()); canvas.drawCircle(x, y, r,
			 * pointAltUI); canvas.drawCircle(x, y, r, point);
			 * 
			 * String id = null;
			 * if(RenderingIcons.containsIcon(o.getSubType())){ id =
			 * o.getSubType(); } else if
			 * (RenderingIcons.containsIcon(o.getType().getDefaultTag() + "_" +
			 * o.getSubType())) { id = o.getType().getDefaultTag() + "_" +
			 * o.getSubType(); } if(id != null){ Bitmap bmp =
			 * RenderingIcons.getIcon(view.getContext(), id); if(bmp != null){
			 * canvas.drawBitmap(bmp, x - bmp.getWidth() / 2, y -
			 * bmp.getHeight() / 2, paintIcon); } } }
			 * 
			 * if (view.getSettings().SHOW_POI_LABEL.get()) { TIntHashSet set =
			 * new TIntHashSet(); for (Amenity o : objects) { int x =
			 * view.getRotatedMapXForPoint(o.getLocation().getLatitude(),
			 * o.getLocation().getLongitude()); int y =
			 * view.getRotatedMapYForPoint(o.getLocation().getLatitude(),
			 * o.getLocation().getLongitude()); int tx =
			 * view.getMapXForPoint(o.getLocation().getLongitude()); int ty =
			 * view.getMapYForPoint(o.getLocation().getLatitude()); String name
			 * = o.getName(view.getSettings().USE_ENGLISH_NAMES.get()); if (name
			 * != null && name.length() > 0) { int lines = 0; while (lines <
			 * TEXT_LINES) { if (set.contains(division(tx, ty, 0, lines)) ||
			 * set.contains(division(tx, ty, -1, lines)) ||
			 * set.contains(division(tx, ty, +1, lines))) { break; } lines++; }
			 * if (lines == 0) { // drawWrappedText(canvas, "...",
			 * paintTextIcon.getTextSize(), x, y + r + 2 +
			 * paintTextIcon.getTextSize() / 2, 1); } else {
			 * drawWrappedText(canvas, name, paintTextIcon.getTextSize(), x, y +
			 * r + 2 + paintTextIcon.getTextSize() / 2, lines); while (lines >
			 * 0) { set.add(division(tx, ty, 1, lines - 1));
			 * set.add(division(tx, ty, -1, lines - 1)); set.add(division(tx,
			 * ty, 0, lines - 1)); lines--; } }
			 * 
			 * } } } }
			 */
	...
	private void drawWrappedText(Canvas cv, String text, float textSize,
			float x, float y, int lines) {
		if (text.length() > TEXT_WRAP) {
			...
			while (pos < end && (line < lines)) {
				...
				if (lastSpace == -1) {
					drawShadowText(cv, text.substring(start, pos), x, y + line
							* (textSize + 2));
					start = pos;
				} else {
	...
	private void drawShadowText(Canvas cv, String text, float centerX,
			float centerY) {
	...
	public OnClickListener getActionListener(List<String> actionsList, Object o) {
		...
		if (a.getPhone() != null) {
			actionsList.add(this.view.getResources().getString(
					R.string.poi_context_menu_call));
		}
		if (a.getSite() != null) {
			actionsList.add(this.view.getResources().getString(
					R.string.poi_context_menu_website));
		}
		if (a.getDescription() != null) {
			actionsList.add(this.view.getResources().getString(
					R.string.poi_context_menu_showdescription));
		}
	...
	private void showDescriptionDialog(Amenity a) {
		...
		bs.setTitle(OsmAndFormatter.getPoiSimpleFormat(a, view.getContext(),
	...
	public String getObjectDescription(Object o) {
		if (o instanceof Amenity) {
			return buildPoiInformation(new StringBuilder(), (Amenity) o)
					.toString();
		}
		return null;
	}
	 ...
	 *            the objects to set
	 */
	public void setPois(ArrayList<MapPoi> pois, boolean isMapPoi) {
		this.pois = pois;
		this.isMapPoi = isMapPoi;
	}

	private void drawItineraryPoi(Canvas canvas) {
		int r = getRadiusPoi(view.getZoom());
		for (MapPoi poi : pois) {
			if (!poi.isDuplicate()) {
				int x = view.getRotatedMapXForPoint(poi.getLatitude(),
						poi.getLongitude());
				int y = view.getRotatedMapYForPoint(poi.getLatitude(),
						poi.getLongitude());
				...
				float textSize = 12 * dm.density;
				paint.setTextSize(textSize);
				paint.setTextAlign(Align.CENTER);
				canvas.drawText(poi.getDisplaySequenceNumber(), x, y
						+ (textSize / 2), paint);
			}
		}
	}

	public void getPoiFromPoint(PointF point, List<MapPoi> poiList) {
		if (pois != null) {
			int ex = (int) point.x;
			int ey = (int) point.y;
			int compare = getRadiusPoi(view.getZoom());
