...
public class OsmBugsLayer extends OsmandMapLayer implements
		ContextMenuLayer.IContextMenuProvider, DialogProvider {

	private static final Log log = OsmLogUtil.getLog(OsmBugsLayer.class);
	...
	private Pattern patternToParse = Pattern
			.compile("putAJAXMarker\\((\\d*), (-?(?:\\d|\\.)+), (-?(?:\\d|\\.)+), '([^']*)', (\\d)\\);"); //$NON-NLS-1$
	...
	public void onDraw(Canvas canvas, RectF latLonBounds, RectF tilesRect,
		...
		if (view.getZoom() >= startZoom) {
			// request to load
			requestToLoad(latLonBounds.top, latLonBounds.left,
					latLonBounds.bottom, latLonBounds.right, view.getZoom());
			for (OpenStreetBug o : objects) {
				int x = view.getMapXForPoint(o.getLongitude());
				int y = view.getMapYForPoint(o.getLatitude());
				canvas.drawCircle(x, y, getRadiusBug(view.getZoom()), o
						.isOpened() ? pointOpenedUI : pointClosedUI);
			}
	...
	public void requestToLoad(double topLatitude, double leftLongitude,
			double bottomLatitude, double rightLongitude, final int zoom) {
		boolean inside = cTopLatitude >= topLatitude
		...
		if ((!inside || (czoom != zoom && objects.size() >= SEARCH_LIMIT))
			...
			final double nTopLatitude = topLatitude
					+ (topLatitude - bottomLatitude);
			final double nBottomLatitude = bottomLatitude
					- (topLatitude - bottomLatitude);
			final double nLeftLongitude = leftLongitude
					- (rightLongitude - leftLongitude);
			final double nRightLongitude = rightLongitude
					+ (rightLongitude - leftLongitude);
			Message msg = Message.obtain(handlerToLoop, new Runnable() {
				@Override
				public void run() {
					if (handlerToLoop != null && !handlerToLoop.hasMessages(1)) {
						boolean inside = cTopLatitude >= nTopLatitude
								&& cLeftLongitude <= nLeftLongitude
								&& cRightLongitude >= nRightLongitude
								&& cBottomLatitude <= nBottomLatitude;
						if (!inside || czoom != zoom) {
							objects = loadingBugs(nTopLatitude, nLeftLongitude,
									nBottomLatitude, nRightLongitude);
		...
		if (objects != null && view != null) {
			...
			try {
				for (int i = 0; i < objects.size(); i++) {
					...
					int x = view.getRotatedMapXForPoint(n.getLatitude(), n
							.getLongitude());
					int y = view.getRotatedMapYForPoint(n.getLatitude(), n
							.getLongitude());
					if (Math.abs(x - ex) <= radius
							&& Math.abs(y - ey) <= radius) {
						radius = small;
						res.add(n);
					}
				}
	...
	public boolean onSingleTap(PointF point) {
		...
		if (!list.isEmpty()) {
			...
			for (OpenStreetBug o : list) {
				...
				res.append(activity.getString(R.string.osb_bug_name)
						+ " : " + o.getName()); //$NON-NLS-1$
			}
	...
	public boolean createNewBug(double latitude, double longitude, String text,
			String authorName) {
	...
	protected List<OpenStreetBug> loadingBugs(double topLatitude,
	...
	private void openBugAlertDialog(final double latitude,
			final double longitude, String message, String authorName) {
	...
	private void prepareOpenBugDialog(Dialog dlg, Bundle args) {
		((EditText) dlg.findViewById(R.id.BugMessage)).setText(args
				.getString(KEY_MESSAGE));
		((EditText) dlg.findViewById(R.id.AuthorName)).setText(args
				.getString(KEY_AUTHOR));
	}

	private Dialog createOpenBugDialog(final Bundle args) {
		final View openBug = activity.getLayoutInflater().inflate(
				R.layout.open_bug, null);
		...
		builder.setPositiveButton(R.string.default_buttons_add,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						String text = ((EditText) openBug
								.findViewById(R.id.BugMessage)).getText()
								.toString();
						String author = ((EditText) openBug
								.findViewById(R.id.AuthorName)).getText()
								.toString();
						// do not set name as author it is ridiculous in that
						// case
						OsmandSettings.getOsmandSettings(activity).USER_OSM_BUG_NAME
								.set(author);
						boolean bug = createNewBug(latitude, longitude, text,
								author);
						if (bug) {
							Toast.makeText(
									activity,
									activity.getResources().getString(
											R.string.osb_add_dialog_success),
	...
	public void openBug(final double latitude, final double longitude) {
		...
		openBugAlertDialog(latitude, longitude, "", settings.USER_OSM_BUG_NAME
				.get());
	}
	...
	private Dialog createCommentBugDialog(final Bundle args) {
		...
		((EditText) view.findViewById(R.id.AuthorName)).setText(OsmandSettings
				.getOsmandSettings(activity).USER_OSM_BUG_NAME.get());
		builder.setNegativeButton(R.string.default_buttons_cancel, null);
		builder.setPositiveButton(R.string.osb_comment_dialog_add_button,
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						OpenStreetBug bug = (OpenStreetBug) args
								.getSerializable(KEY_BUG);
							...
							Toast
									.makeText(
											...
											activity
													.getResources()
													.getString(
															R.string.osb_comment_dialog_success),
											Toast.LENGTH_LONG).show();
							clearCache();
	...
	private Dialog createCloseBugDialog(final Bundle args) {
		...
		builder.setNegativeButton(R.string.default_buttons_cancel, null);
		builder.setPositiveButton(R.string.osb_close_dialog_close_button,
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						OpenStreetBug bug = (OpenStreetBug) args
						...
						if (closed) {
							Toast
									.makeText(
											activity,
											activity
													.getString(R.string.osb_close_dialog_success),
											Toast.LENGTH_LONG).show();
							clearCache();
							refreshMap();
						} else {
							Toast
									.makeText(
											activity,
											activity
													.getString(R.string.osb_close_dialog_error),
											Toast.LENGTH_LONG).show();
						}
					}
				});
	...
	public String getObjectDescription(Object o) {
		if (o instanceof OpenStreetBug) {
			return activity.getString(R.string.osb_bug_name)
					+ " : " + ((OpenStreetBug) o).getName(); //$NON-NLS-1$
		}
		return null;
	}
	...
	public LatLon getObjectLocation(Object o) {
		if (o instanceof OpenStreetBug) {
			return new LatLon(((OpenStreetBug) o).getLatitude(),
					((OpenStreetBug) o).getLongitude());
		}
		return null;
	}
