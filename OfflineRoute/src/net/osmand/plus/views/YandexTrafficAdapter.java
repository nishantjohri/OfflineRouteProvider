...
public class YandexTrafficAdapter extends MapTileAdapter {
	...
	private final static Log log = OsmLogUtil.getLog(MapTileLayer.class);
	...
	@Override
	public void onInit() {
		Toast.makeText(view.getContext(), R.string.thanks_yandex_traffic,
	...
	protected void updateTimeStampImpl() {
		if (mTimestamp == null
				|| (System.currentTimeMillis() - lastTimestampUpdated) > DELTA) {
			log.info("Updating timestamp"); //$NON-NLS-1$
			try {
				BufferedInputStream in = new BufferedInputStream(
						new URL("http://jgo.maps.yandex.net/trf/stat.js").openStream(), 1024); //$NON-NLS-1$
				...
				if (!newTimestamp.equals(mTimestamp)) {
					...
					TileSourceTemplate template = new TileSourceTemplate(
							YANDEX_PREFFIX + mTimestamp,
							"http://jgo.maps.yandex.net/tiles?l=trf&x={1}&y={2}&z={0}&tm="
									+ mTimestamp, ".png", 17, 7, 256, 8, 18000);
	...
	private void clearCache() {
		File dir = view.getSettings().extendOsmandPath(
				ResourceManager.TILES_PATH);
