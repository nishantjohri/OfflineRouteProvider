...
public class AnimateDraggingMapThread {
	...
	protected static final Log log = OsmLogUtil
			.getLog(AnimateDraggingMapThread.class);
	...
	private void pendingRotateAnimation() {
		...
		do {
			...
			float rotationDiff = MapUtils.unifyRotationDiff(tileView
					.getRotate(), targetRotate);
			...
			if (absDiff > 0) {
				...
				if (absDiff < 1) {
					...
					tileView.rotateToAnimate(rotationDiff / 5
							+ tileView.getRotate());
				}
			}
	...
	public void startMoving(final double finalLat, final double finalLon,
			final int endZoom, final boolean notifyListener) {
		stopAnimatingSync();
		...
		float mStX = (float) ((MapUtils.getTileNumberX(mZoom, startLon) - MapUtils
				.getTileNumberX(mZoom, finalLon)) * tileSize);
		float mStY = (float) ((MapUtils.getTileNumberY(mZoom, startLat) - MapUtils
				.getTileNumberY(mZoom, finalLat)) * tileSize);
		while (Math.abs(mStX) + Math.abs(mStY) > 1200) {
			mZoom--;
			if (mZoom <= 4) {
				skipAnimation = true;
			}
			mStX = (float) ((MapUtils.getTileNumberX(mZoom, startLon) - MapUtils
					.getTileNumberX(mZoom, finalLon)) * tileSize);
			mStY = (float) ((MapUtils.getTileNumberY(mZoom, startLat) - MapUtils
					.getTileNumberY(mZoom, finalLat)) * tileSize);
		}
		final int moveZoom = mZoom;
		// check if animation needed
		skipAnimation = skipAnimation
				|| (Math.abs(moveZoom - startZoom) >= 3 || Math.abs(endZoom
						- moveZoom) > 3);
		if (skipAnimation) {
		...
		final float mMoveX = FloatMath.cos(rad) * mStX - FloatMath.sin(rad)
				* mStY;
		final float mMoveY = FloatMath.sin(rad) * mStX + FloatMath.cos(rad)
				* mStY;

		final float animationTime = Math.max(450, (Math.abs(mStX) + Math
				.abs(mStY))
				/ 1200f * MOVE_MOVE_ANIMATION_TIME);
		...
		startThreadAnimating(new Runnable() {
			...
			public void run() {
				...
				if (moveZoom != startZoom) {
					animatingZoomInThread(startZoom, moveZoom,
							ZOOM_MOVE_ANIMATION_TIME, notifyListener);
				}

				if (!stopped) {
					animatingMoveInThread(mMoveX, mMoveY, animationTime,
							notifyListener);
				}
				if (!stopped) {
					tileView.setLatLonAnimate(finalLat, finalLon,
							notifyListener);
				}

				if (!stopped && moveZoom != endZoom) {
					animatingZoomInThread(moveZoom, endZoom,
							ZOOM_MOVE_ANIMATION_TIME, notifyListener);
				}
	...
	private void animatingMoveInThread(float moveX, float moveY,
			float animationTime, boolean notify) {
	...
	private void animatingZoomInThread(int zoomStart, int zoomEnd,
			float animationTime, boolean notifyListener) {
		...
		startThreadAnimating(new Runnable() {
			...
			public void run() {
				...
				setTargetValues(zoomEnd, tileView.getLatitude(), tileView
						.getLongitude());
				animatingZoomInThread(zoomStart, zoomEnd, animationTime,
						notifyListener);
				pendingRotateAnimation();
			}
		}); //$NON-NLS-1$
		...
		startThreadAnimating(new Runnable() {
			...
			public void run() {
				...
				while (!stopped) {
					...
					tileView.dragToAnimate(curX, curY, newX, newY,
							notifyListener);
					curX = newX;
					curY = newY;
	...
	public void startRotate(final float rotate) {
		if (!isAnimating()) {
			clearTargetValues();
			// stopped = false;
			// do we need to kill and recreate the thread? wait would be enough
			// as now it
