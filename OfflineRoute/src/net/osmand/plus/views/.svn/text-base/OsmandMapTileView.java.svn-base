...
public class OsmandMapTileView extends SurfaceView implements
	...
	protected static final Log log = OsmLogUtil.getLog(OsmandMapTileView.class);
	/**
	 * MapTree zoom level - could be float to show zoomed tiles
	 */
	...
	public float getTileSize() {
		...
		// for example for device with density > 1 draw tiles the same size as
		// with density = 1
		// It makes text bigger but blurry, the settings could be introduced for
		// that
		if (dm != null && dm.density > 1f
				&& !getSettings().USE_HIGH_RES_MAPS.get()) {
			res *= dm.density;
		}
		return res;
	}
	...
	protected void drawEmptyTile(Canvas cvs, float x, float y, float ftileSize,
		...
		for (int k1 = 0; k1 < emptyTileDivisor; k1++) {
			for (int k2 = 0; k2 < emptyTileDivisor; k2++) {
				...
				if ((k1 + k2) % 2 == 0) {
					cvs.drawRect(xk, yk, xk + tileDiv, yk + tileDiv,
							paintGrayFill);
				} else {
					cvs.drawRect(xk, yk, xk + tileDiv, yk + tileDiv,
							nightMode ? paintBlackFill : paintWhiteFill);
				}
			}
		}
	}
	...
	public void calculateTileRectangle(Rect pixRect, float cx, float cy,
			float ctilex, float ctiley, RectF tileRect) {
	...
	public void calculatePixelRectangle(Rect pixelRect, float cx, float cy,
			float ctilex, float ctiley, RectF tileRect) {
		float x1 = calcDiffPixelX(tileRect.left - ctilex, tileRect.top - ctiley);
		float x2 = calcDiffPixelX(tileRect.left - ctilex, tileRect.bottom
				- ctiley);
		float x3 = calcDiffPixelX(tileRect.right - ctilex, tileRect.top
				- ctiley);
		float x4 = calcDiffPixelX(tileRect.right - ctilex, tileRect.bottom
				- ctiley);
		float y1 = calcDiffPixelY(tileRect.left - ctilex, tileRect.top - ctiley);
		float y2 = calcDiffPixelY(tileRect.left - ctilex, tileRect.bottom
				- ctiley);
		float y3 = calcDiffPixelY(tileRect.right - ctilex, tileRect.top
				- ctiley);
		float y4 = calcDiffPixelY(tileRect.right - ctilex, tileRect.bottom
				- ctiley);
	...
	private void refreshMapInternal(boolean force) {
		...
		boolean useInternet = getSettings().USE_INTERNET_TO_DOWNLOAD_TILES
				.get();
		if (useInternet) {
			if (application != null) {
				application.getResourceManager().getMapTileDownloader()
						.refuseAllPreviousRequests();
			}
		}
		...
		synchronized (holder) {
			...
			if (canvas != null) {
				...
				if (application != null) {
					Boolean dayNightRenderer = application.getDaynightHelper()
							.getDayNightRenderer();
				...
				try {
					...
					calculateTileRectangle(boundsRect, w, h, tileX, tileY,
							tilesRect);
					latlonRect.top = (float) MapUtils.getLatitudeFromTile(
							nzoom, tilesRect.top);
					latlonRect.left = (float) MapUtils.getLongitudeFromTile(
							nzoom, tilesRect.left);
					latlonRect.bottom = (float) MapUtils.getLatitudeFromTile(
							nzoom, tilesRect.bottom);
					latlonRect.right = (float) MapUtils.getLongitudeFromTile(
							nzoom, tilesRect.right);
					if (nightMode) {
					...
					// int height = (int) FloatMath.ceil(tilesRect.bottom -
					// top);
					...
					// }
					// }
					drawOverMap(canvas, latlonRect, tilesRect,
							new DrawSettings(nightMode, force));

					// log.info("Draw with layers " +
	...
	private void drawOverMap(Canvas canvas, RectF latlonRect, RectF tilesRect,
		...
		for (int i = 0; i < layers.size(); i++) {
			...
			// long time = System.currentTimeMillis();
			// log.debug("Layer time " + (time - prev) + " " +
			// zOrders.get(layers.get(i)));
			// prev = time;
		}
		canvas.restore();
		if (showMapPosition) {
	...
	public boolean mapIsAnimating() {
		return animatedDraggingThread != null
				&& animatedDraggingThread.isAnimating();
	}
	...
	public int getRotatedMapXForPoint(double latitude, double longitude) {
		...
		return (int) (calcDiffPixelX((float) (xTile - getXTile()),
				(float) (yTile - getYTile())) + cx);
	}
	...
	public int getRotatedMapYForPoint(double latitude, double longitude) {
		...
		double xTile = MapUtils.getTileNumberX(getZoom(), longitude);
		double yTile = MapUtils.getTileNumberY(getZoom(), latitude);
		return (int) (calcDiffPixelY((float) (xTile - getXTile()),
				(float) (yTile - getYTile())) + cy);
	}
	...
	public boolean isPointOnTheRotatedMap(double latitude, double longitude) {
		...
		double xTile = MapUtils.getTileNumberX(getZoom(), longitude);
		double yTile = MapUtils.getTileNumberY(getZoom(), latitude);
		int newX = (int) (calcDiffPixelX((float) (xTile - getXTile()),
				(float) (yTile - getYTile())) + cx);
		int newY = (int) (calcDiffPixelY((float) (xTile - getXTile()),
				(float) (yTile - getYTile())) + cy);
	...
	public void moveTo(float dx, float dy) {
		...
		this.latitude = MapUtils
				.getLatitudeFromTile(getZoom(), getYTile() + fy);
		this.longitude = MapUtils.getLongitudeFromTile(getZoom(), getXTile()
				+ fx);
		refreshMap();
	...
	public LatLon getLatLonFromScreenPoint(float x, float y) {
		...
		float fy = calcDiffTileY(dx, dy);
		float fx = calcDiffTileX(dx, dy);
		double latitude = MapUtils.getLatitudeFromTile(getZoom(), getYTile()
				+ fy);
		double longitude = MapUtils.getLongitudeFromTile(getZoom(), getXTile()
				+ fx);
	...
	private class MapTileViewMultiTouchZoomListener implements
		...
		public void onZoomStarted(float distance, PointF centerPoint) {
			...
			initialMultiTouchLocation = getLatLonFromScreenPoint(centerPoint.x,
					centerPoint.y);
		...
		private void zoomPositionChanged(float calcZoom) {
			...
			int z = (int) calcZoom;
			double tx = MapUtils.getTileNumberX(z, initialMultiTouchLocation
					.getLongitude());
			double ty = MapUtils.getTileNumberY(z, initialMultiTouchLocation
					.getLatitude());
	...
	private class MapTileViewOnGestureListener implements OnGestureListener {
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
				float velocityY) {
			animatedDraggingThread.startDragging(velocityX, velocityY, e1
					.getX(), e1.getY(), e2.getX(), e2.getY(), true);
			return true;
		}
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2,
			...
			dragToAnimate(e2.getX() + distanceX, e2.getY() + distanceY, e2
					.getX(), e2.getY(), true);
			return true;
		}
	...
	private class MapTileViewOnDoubleTapListener implements OnDoubleTapListener {
		...
		public boolean onDoubleTap(MotionEvent e) {
			...
			getAnimatedDraggingThread().startMoving(l.getLatitude(),
					l.getLongitude(), getZoom() + 1, true);
			return true;
		}
