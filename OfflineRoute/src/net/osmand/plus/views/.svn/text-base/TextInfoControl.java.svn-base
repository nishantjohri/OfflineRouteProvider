...
public class TextInfoControl extends MapInfoControl {
	...
	public TextInfoControl(Context ctx, int leftMargin, Paint textPaint,
			Paint subtextPaint) {
		super(ctx);
	...
	protected void onWLayout(int w, int h) {
		if (imageDrawable != null) {
			...
			imageDrawable.setBounds(0, (int) (3 * scaleCoefficient),
					imageDrawable.getMinimumWidth(), imageDrawable
							.getMinimumHeight()
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (text != null) {
			...
			h = (int) (5 * scaleCoefficient + Math.max(textPaint.getTextSize(),
	...
	protected void onDraw(Canvas cv) {
		...
		if (isVisible()) {
			...
			margin += leftMargin;
			drawShadowText(cv, text, margin, getWHeight() - 3
					* scaleCoefficient, textPaint);
			if (subtext != null) {
				drawShadowText(cv, subtext, margin + 2 * scaleCoefficient
						+ textPaint.measureText(text), getWHeight() - 3
